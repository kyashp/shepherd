...
const backgroundTestMissions: Array<{ service: ShepherdService; missionId: string }> = [];
  ...
  const makeDeletable = async (directory: string): Promise<void> => {
    ...
    for (const name of await readdir(directory)) {
      ...
      if (entryStat.isDirectory() && !entryStat.isSymbolicLink()) {
        ...
        if (canonicalEntry !== entry || !canonicalEntry.startsWith(canonicalRoot + path.sep)) {
...
async function startTrackedTestMission(service: ShepherdService): Promise<{ missionId: string }> {
...
afterEach(async () => {
  while (backgroundTestMissions.length > 0) {
    ...
    if (state !== "completed" && state !== "failed" && state !== "attention_required" && state !== "cancelled") {
...
describe("Shepherd deterministic walking skeleton", { timeout: 15_000 }, () => {
