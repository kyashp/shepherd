...
test("Project Group initializes safely, pairs bounded requests, and reports verified Agent summaries", async ({ page, request }, testInfo) => {
  ...
  await composer.fill('@"Group Frontend" use an HttpOnly session cookie; $(cat .env)');
  ...
  await composer.fill('@"Group Backend" use a bearer JWT.');
  ...
  await expect(page.locator(".group-message").filter({ hasText: "use an HttpOnly session cookie" }).getByRole("link", { name: /Contract/u })).toBeVisible();
