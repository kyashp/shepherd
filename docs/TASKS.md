...
| `OPS-06` | 6.1, 8.10, 17 baseline | **BLOCKED by `TST-12`**; rebased candidate `34477e8` remains unintegrated; collaborator/macOS validation required | The runner returns a closed bounded stage/reason result and keeps every hardening gate fail-closed. Auditor focused 186/186 and literal check (launcher 3/3, Server 601 plus two explicit skips, Web 17/17, strict/build) passed. Independent security review found the executor mount-cleanup branch still attaches raw `cleanupError` as `Error.cause`; an uncaught startup error may expose private filesystem/OS detail, so the broad bounded-output claim is not yet true. No live/model call ran. `T,A,C,S,L,I`. | **65% scoped**; functional gates pass; security `S`, affected-macOS `L`, integration `I` pending |
| `TST-12` | 0.3, 6.1, 13 | **READY**; Fixer; owns executor cleanup error/test only | Remove raw executor preflight mount-cleanup cause from startup-visible error. Add a causal failure injection containing planted private path/secret/OS detail and prove only the fixed bounded message reaches rejection/startup output. Preserve runner typed diagnostics, cleanup attempt/order, fail-closed startup and all sandbox gates; no broad logging/refactor or swallowed failure. Acceptance: focused executor/runner mapping and cleanup suite, startup-output leak assertion, adjacent live-runtime/config, strict/literal check and security re-review. | **0% fix confidence**; Medium RED source-confirmed; not audited |
