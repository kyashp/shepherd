...
| `F-06` | 9.1, 12: persistence error | **FIXER STACK / Auditor pending** with TST-21–24 | Representative recovery design, raw-read closure and replacement private-directory marker state machine pass focused, combined, security and two literal full gates after deterministic fixture quiescence. | **95% scoped**; `T,C,S` candidate evidence; `A,I` pending |
...
| `TST-22` | 0.3, 9.1, 12, 13 | **FIXER CANDIDATE**; Auditor pending | Owner-validated `0700` private temp namespace plus pre-published fixed-schema marker makes publication/removal prefixes idempotent without touching general-parent lookalikes. Store 70/70, security 0 High/Medium, combined 138/138 and two full gates pass after TST-23/24 quiescence. | **95% scoped**; `T,C,S` candidate evidence; `A,I` pending |
| `TST-23` | 0.2, 9.1, 12, 16 Phase 9B | **FIXER CANDIDATE**; Auditor pending; test-only | One no-op Store sentinel after attention observation joins all prior original-store work before reload/teardown. Exact row 20/20, combined 138/138 and two full gates pass without late rejection/artifact. | **95% scoped**; `T,C` pass; `A,I` pending |
| `TST-24` | 0.2, 9.1, 12, 16 Phase 9B | **FIXER CANDIDATE**; Auditor pending; test-only | Bounded wait for the exact recovery intent to clear plus a no-op Store sentinel joins reconciliation before durable reads/teardown. Exact row 30/30, combined 138/138 and two full gates pass. | **95% scoped**; `T,C` pass; `A,I` pending |
