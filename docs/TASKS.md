...
| `F-04` | 12: worktree creation | **BLOCKED by `TST-17`** on audit branch `audit/f04-plane-unwind`; Fixer | First-creation bounded failure and real partial-Git rollback pass, but an independently reproduced second-of-two creation failure leaves the first durable `ready` Plane, Contract association, worktree and branch attached to a terminal Mission. Unwind every earlier Plane in the initial creation batch while preserving the second Contract's bounded failure and original cause precedence. | **75% scoped**; first-call T/C/S pass, batch cleanup/I fail; not audited |
| `TST-17` | 0.3, 6.1, 12 | **CONFIRMED RED** at candidate `5cc24f5`; Fixer | Inject failure at the second `contract_plane_creation_start`. Require zero durable Planes/planeIds and zero attached worktrees/branches for the batch, exact owning Contract/Mission `worktree_creation_failure` evidence, Agent/project release, no executor/verifier/integration/candidate/promotion, protected HEAD/canary unchanged, bounded public/reload/log output, and exact cleanup of only batch-owned resources. | **0%**; RED 1/1; independent security Medium; not audited |
