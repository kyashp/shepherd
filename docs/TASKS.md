...
| `F-05` | 8.4, 12: Git conflict | **WORKER VERIFIED candidate** on `work/mock-main`; Auditor pending | A real ten-file textual conflict now persists bounded `git_conflict/integration_merge` Mission, failed integration Plane and event/API evidence. Merge abort retains a clean integration worktree for inspection; reset removes every retained Plane worktree/branch. Source Contracts remain verified, Agents release, and candidates/promotion/protected HEAD remain unchanged. | **95% scoped**; `T,A,C,S` 4/5 local pass; `I` pending |
