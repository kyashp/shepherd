...
| `TST-03` | 0.2, 12, 16 Phase 9B, 17 | **READY — Fixer; blocks hosted baseline and UI-03 final `C`** | Hosted Node 22 run `33261198788` timed out the F-03 thrown-verifier blocked-sibling case at 30s and reported an unhandled `ContractVerificationInfrastructureError` (558 passed, 5 skipped, 1 failed). Source inspection shows the test awaits `siblingEntered` before attaching a rejection assertion to `run`, while the throwing frontend can terminalize before the backend enters its verifier; no `finally` releases the sibling on early failure/timeout. Five isolated local runs passed, confirming schedule sensitivity rather than correction. Add a deterministic two-arrival barrier/early rejection handling and unconditional bounded release/join in the test fixture only; preserve production F-03 semantics. Require causal RED/GREEN, both parameter rows, repeated focused/full contention, literal local check and hosted Node 22 success. `T,C,I`. | **100% diagnosis, 0% correction**; 0/3; not audited |
