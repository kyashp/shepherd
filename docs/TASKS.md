...
| `E2E-01` | 11.6.1, 17 baseline | `BLOCKED` by `TST-05` and `TST-06`; candidate `592699d140145165c5ca0d9511478171e5051271` is unintegrated | Candidate behavior passed deterministic harness unit 3/3, Chromium 6/6, strict test typecheck and literal check; its separately bounded live legacy gate passed exactly 2/2 turns without retry. Independent audit found non-reproducible/overclaimed visual evidence and security/cleanup failure-path gaps. Correct only the harness, then re-audit the authenticated create → artifact/test → follow-up/session → restart-continuity journey. `C,B,L,S,U,I`. | **70% scoped implementation evidence**; blocked and not audited |
| `TST-05` | 0.4, 11.6.1, 11.7 | `READY`; Fixer; blocks E2E-01 | Routine harness must leave tracked evidence and unrelated UI-03 images untouched; use ignored routine output plus an explicit reproducible review-update path. Record port-down as infrastructure evidence, not a blank rendered state. Prove Create action reachability and actual Tab/focus-visible/lifecycle keyboard behavior at both viewports without production UI/design changes. `T,C,B,U,I`. | **0%**; 0/5; not audited |
| `TST-06` | 0.3, 11.6.1, 13 | `READY`; Fixer; blocks E2E-01 | Secret-safe assertions, cleanup of a previously retained root after stop/restart failure, canonical repository-contained managed ancestors before create/remove, and safe opt-in live failure artifacts. Add causal fault tests and preserve the allowlisted environment/filesystem authority. `T,C,S,U,I`. | **0%**; 0/5; not audited |
