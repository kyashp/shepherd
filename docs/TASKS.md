...
| `F-05` | 8.4, 12: Git conflict | **BLOCKED by `TST-18`** on `audit/f05-conflict-abort`; Fixer | Ordinary real ten-file conflict, bounded preview and reset pass, but a merge-abort cleanup exception masks the detected conflict, escapes the Git boundary and can yield `unknown`/raw diagnostics while leaving the retained integration Plane unproven clean. Preserve conflict as primary and truthfully represent cleanup failure. | **75% scoped**; ordinary T/A/C/S pass, double-fault/I fail; not audited |
| `TST-18` | 0.3, 8.4, 12 | **CONFIRMED RED** at candidate `a7b669b`; Fixer | Inject enumeration/path-validation and thrown/accepted-nonzero merge-abort failures after a real conflict. Require abort/restore in a mandatory cleanup boundary, exact clean/head proof, primary bounded `git_conflict/integration_merge`, and no raw diagnostics. If cleanup cannot be proven, retain explicit bounded attention evidence without claiming a clean Plane. Preserve verified Contracts, Agent release, protected HEAD/canary, no downstream work, and exact reset cleanup. | **0%**; RED 1/1 plus source-confirmed failure slices; independent security Medium; not audited |
