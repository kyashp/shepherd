...
| `E2E-01` | 11.6.1, 17 baseline | `CANDIDATE`; corrected harness pending Auditor/hosted review | Deterministic harness unit passed 6/6 and Chromium passed 6/6 at both required viewports. The authenticated create → artifact/test 2/2 → follow-up/session → port-down → restart-continuity journey passed with actual keyboard Create/Stop/Start activation; 16 ephemeral stage captures were inspected. Strict test typecheck and literal check passed twice. Ordinary runs left tracked evidence byte-identical. The separately bounded earlier live legacy gate remains exactly 2/2 turns without retry; no live call was repeated here. `C,B,L,S,U,I`. | **90% scoped candidate evidence**; `C,B,S` passed plus prior bounded `L`; `U,I` pending; not audited |
| `TST-05` | 0.4, 11.6.1, 11.7 | `CANDIDATE`; Fixer correction pending Auditor/hosted review | Default screenshots route only to ignored `.tmp/playwright-evidence`; explicit `test:e2e:starter-kit:evidence` is the non-default docs update path. Port closure is asserted as infrastructure and no blank canvas is captured. At both viewports Create is scrolled into the existing inner viewport, visibly focused and activated with Tab/Enter; Stop/Start use the same keyboard path. Harness 6/6, Chromium 6/6, 16 stage captures inspected, tracked screenshot hashes unchanged, and two literal checks passed. No CSS/UI/theme change. `T,C,B,U,I`. | **96% scoped candidate**; `T,C,B` passed (3/5); `U,I` pending; not audited |
| `TST-06` | 0.3, 11.6.1, 13 | `CANDIDATE`; Fixer correction pending Auditor/hosted review | Assertions compare only safe booleans; live trace/screenshot/video are off; exact opt-in, two-turn/no-retry cap, loopback/auth/allowlisted environment and empty live `SHEPHERD_MODEL` remain. Managed repository/`.tmp`/harness/run-root identities are lstat/realpath checked before allocation/removal; fault tests reject a repo-contained ancestor symlink, prove later cleanup of a retained stopped root after restart failure, and prove fresh setup failure leaves no allocated root. Harness 6/6, strict test typecheck and two literal checks passed; no run roots remained. `T,C,S,U,I`. | **94% scoped candidate**; `T,C,S` passed (3/5); `U,I` pending; not audited |
...
- **49 tracked work items:** 9 audited, 3 externally held, and 37 other incomplete
  items across P0/P1/P2.
- **Next ready worker assignment:** `F-01/02` unless the integrator selects another
  non-overlapping ready row.
- **First fixer assignment:** `TST-05`, followed by or coordinated narrowly with
  `TST-06`; both block E2E-01. `OPS-06` still requires collaborator/macOS evidence.
