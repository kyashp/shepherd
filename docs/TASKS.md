...
| `F-05` | 8.4, 12: Git conflict | **FIXER CANDIDATE; Auditor pending** | TST-18 post-conflict cleanup remains intact; pre-merge identity now proves no merge in progress, clean state, exact persisted branch and exact persisted head before mutation. Five real-Git mismatch states retain all target/source refs and worktree state with fixed attention evidence. | **95% scoped**; local T/C/S pass, hosted/Auditor I pending |
...
| `TST-19` | 0.3, 8.4, 12 | **FIXER CANDIDATE; Auditor pending** | Detached, sibling branch, moved head, dirty target and preexisting merge real-Git rows fail before mutation through fixed cause-free cleanup evidence. Target HEAD/status/all head refs and source HEAD remain exact; existing service attention/no-downstream mapping is preserved. | **90% scoped**; focused repeated locally, full/security pending; not audited |
