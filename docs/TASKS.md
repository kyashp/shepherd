...
| ID | PRD ref | Status / owner / dependencies | Evidence and required acceptance | Confidence; gates passed; audit |
|---|---|---|---|---|
| `F-01/02` | 12: Agent timeout/runtime | **CORRECTED CHAIN CANDIDATE through `TST-15`**; Auditor integration pending | Closed typed Runtime identity and TST-13 public-message boundary pass across Contract/candidate durable/API surfaces. TST-14 preserves execution-home cleanup precedence/reconciliation; TST-15 now bounds sentinel and interrupted-home/workspace filesystem failures through one fixed policy with contained retry. Candidate gates: executor 33/33, focused/adjacent 120/120, two literal checks and final scoped security PASS. No live/model/UI call ran. | **95% scoped**; local T/C/S pass through TST-15; Auditor U/I pending |
...
| `TST-15` | 0.3, 6.1, 12, 13 | **FIX CANDIDATE** on `fix/f-01-02-sentinel-cleanup`; Auditor integration pending | One closed internal stage/reason policy bounds sentinel pre-open/open/body/close/unlink and exact private-home/preflight-workspace reconciliation rm. Primary precedence, close-only, triple/double faults, repeated same-process exact retry, retained artifact, startup Error/cause/store/log and containment tests pass. Restart with invalid retained sentinel fails closed and may require operator cleanup; cross-process retry is not claimed. Executor 33/33, focused/adjacent 120/120, two literal checks and final security review pass; external calls 0. | **98% scoped**; no High/Medium/material gap; Auditor U/I pending |
