...
| ID | PRD ref | Status / owner / dependencies | Evidence and required acceptance | Confidence; gates passed; audit |
|---|---|---|---|---|
| `F-01/02` | 12: Agent timeout/runtime | **BLOCKED by `TST-13`**; candidate `685e4ff` remains unintegrated | Shared typed Runtime identity correctly distinguishes timeout from execution across Contract/Plane/Agent/Mission/event/API/reload; releases ownership; preserves untyped `unknown`, cancellation/F-03/candidate/verifier paths; and creates no candidate/promotion. Auditor passed focused 72/72, adjacent 66/66 and literal check (launcher 3/3, Server 606 plus two explicit skips, Web 17/17, strict/build). Security review found arbitrary parsed Runtime stderr still becomes the typed message and is persisted/public across every failure surface; configured-secret/path redaction is insufficient. No live/model/UI call ran. `T,A,C,S,I`. | **75% scoped**; causal mapping passes; security/public-message and integration blocked by `TST-13` |
| `TST-13` | 0.3, 12, 13 | **READY**; Fixer; owns Runtime failure public-message boundary/tests | Keep typed `timeout`/`execution` identity but never carry arbitrary parsed error/stderr into the throwable consumed by Shepherd. Use fixed bounded public execution text; timeout may retain only its validated numeric deadline. Add real runner→executor→service opaque-stderr/secret/path canaries absent from throwable surfaces, Contract/Plane/Mission, Agent, event, store/reload and public DTO. Preserve cancellation precedence, untyped unknown, candidate verifier errors, F-03, OPS-06 and authority release. Acceptance: RED/GREEN causal chain plus focused/adjacent/full check, scans and security re-review. | **0% fix confidence**; Medium RED source-confirmed; not audited |
