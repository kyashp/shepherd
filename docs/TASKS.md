...
| `CHAT-02` | 4.1.2, 8.1, 11.5–11.6.3 | **REVIEWED CANDIDATE** on [PR #59](https://github.com/kyashp/shepherd/pull/59), [issue #58](https://github.com/kyashp/shepherd/issues/58); stacked on PR #57 | Every Agent role defaults to Shepherd-managed chat with a visible direct-Playground opt-out. A bounded deterministic planner requires a concrete objective, 1–8 writable project-relative artifacts and an enforceable whole-statement `Acceptance:` predicate. Missing, negated, compound or authority-incompatible fields persist as an idempotent clarification draft without reserving an Agent or creating a Mission, Contract, or Plane. A sufficient follow-up creates one authority-intersected Contract, Git-isolated Plane, strict manifest, registry-owned independent check, integration Plane, final independent re-verification, expected-HEAD CAS promotion and exact verified-artifact synchronization to the Agent workspace. Existing-project policy changes and post-CAS completion are crash-reconciled without weakening durable promotion evidence. The fixed two-Agent auth collision path remains unchanged. Literal `npm run check` passed launcher 3/3, Server 771 plus two opt-in skips, Web 18/18, strict types and builds; authenticated Chromium passed 6/6 across both viewports for auth collision, clarification→general promotion, causal direct opt-out, human review, workspace output and no document overflow. Independent UI/security re-reviews passed with no High/Medium finding; protected-main integration remains. `T,C,B,S,U,I`. | **98% scoped**; `T,C,B,S,U` passed (5/6), `I` pending; not audited |
...
| `PERF-01` | 14 | **CANDIDATE** on [issue #46](https://github.com/kyashp/shepherd/issues/46), based on protected `main` `baa6b2f`; #41 integrated at `b2285ab` and #43 at `fbc937f` | A deterministic compiled-service/real-Chromium run passed 2/2 at `1280x800` and `1440x900`; tracked collision-visible and final-promotion screenshots are in `docs/ui-review/perf-01/`. Natural samples use the earliest of exactly two post-initial-creation `contract_started` events as a conservative two-Plane-ready upper bound (not raw `Plane.createdAt`), the exact 2/2 verification boundary, visible collision, and `promotion_completed`: 339/412 ms accepted → two-Plane-ready, 334/320 ms verification, 391/74 ms persisted event → visible (limit `<=1.5s`), 2,083/2,187 ms collision → promotion complete, 3,848/4,167 ms total. Separate test-owned `ShepherdService` executor instrumentation proves two actual candidate execution intervals overlap before release and is explicitly not timing evidence. Browser fixture ledger assertions require 11 unique exact container lifecycles and an empty directory. A real ContainerVerifier boundary test separately passed no-network/read-only enforcement; neither proof claims package-download detection. Local deterministic evidence only; no live-provider, hosted, or three-run rehearsal claim. `B,I`. | **70%**; `B` passed, `I` pending; not audited |
