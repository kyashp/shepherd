...
| `F-06` | 9.1, 12: persistence error | **WORKER VERIFIED candidate** on `work/mock-main`; Auditor pending | The representative post-Contract/pre-integration transition uses a bounded adjacent intent journal and exact persisted-byte digest. Uncommitted writes reconcile once to `persistence_error`; committed rename/dir-sync ambiguity continues safely; pending recovery blocks unrelated mutations. Corrupt/stale/unknown/mismatched journals fail closed. | **95% scoped**; `T,A,C,S` 4/5 local pass; `I` pending |
