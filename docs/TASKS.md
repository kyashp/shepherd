...
| `CHAT-03` | 4.1.2, 8.1, 11.5–11.6.3, 13 | **REVIEWED CANDIDATE** on [issue #81](https://github.com/kyashp/shepherd/issues/81), branch `fix/81-contract-intake-clarification` | Private-chat intake now fails closed unless the terminal request has one direct, allowlisted action on a quoted authority-contained artifact, the exact bounded project-only `Safety:` statement, and one literal-content `Acceptance:` predicate. Secret/credential, external/production/network, privileged, destructive, safeguard-bypass, conditional, negated, approval-dependent and ambiguous requests stay clarification-only and create no Mission, Contract or Plane. Earlier unsafe intent remains sticky until exact `Replace prior request:` recovery; a complete follow-up may supersede only earlier vague drafts that named no artifact and had no unsafe intent. The UI gives exact corrective syntax without design changes. Live Runtime preflight also proves a parent-only credential canary cannot be recovered from `/proc/*/environ` inside the actual Codex sandbox. Final literal check passed launcher 3/3, Server 824 passed + two opt-in skips, Web 20/20, strict types and both builds; focused parser/service passed 27/27; authenticated Chromium passed 8/8 at 1280×800 and 1440×900 with no page overflow. Independent security and UI reviews passed with no High/Medium finding. An explicitly constrained live Mission reached execution but failed its separate result-manifest contract; this row therefore does not close `LIVE-01`. `T,A,C,B,S,U,I`. | **99% scoped deterministic candidate**; `T,A,C,B,S,U` passed (6/7), integration `I` pending; live end-to-end remains open |
