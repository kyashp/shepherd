...
| `TST-20` | 0.2, 12, 16 Phase 9B, 17 | **FIXER CANDIDATE; hosted Auditor pending** | Exact row attaches rejection observation immediately, releases and joins the blocked backend checkpoint lifecycle in `finally`, then awaits a test-owned no-op store mutation before teardown. Fail-fast, no sibling verifier invocation, durable states and no-promotion assertions remain. | **90%**; exact row 20/20 local plus strict types; full/hosted pending |
