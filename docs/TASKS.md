...
| `GC-03/04` | 8.1, 11.5–11.6.3 | `PARTIAL`; exact Project Group path blocked by `GC-02/05`; [issue #41](https://github.com/kyashp/shepherd/issues/41) | PR #57 replaces the former Shepherd selector alternate with the independently reviewed private-Agent `CHAT-01` flow. That candidate does not complete the required Project Group `@Agent` click/type → targeted Contract → verified manifest-derived Agent summary journey. Finish the Project Group path without arbitrary shell/model authority. `T,A,C,B,S,U,I`. | **35%**; private-chat alternate reviewed locally, required journey 0/7; not fully audited |
