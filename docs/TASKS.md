...
| `TST-03` | 0.2, 12, 16 Phase 9B, 17 | **READY — Fixer; blocks hosted baseline and UI-03 final `C`** | Hosted Node 22 run `33261198788` timed out the F-03 thrown-verifier blocked-sibling case at 30s and reported an unhandled `ContractVerificationInfrastructureError` (558 passed, 5 skipped, 1 failed). A disposable controlled mutation held the backend at `contract_verification_snapshot_ready` and shortened only the temporary budget: it deterministically reproduced the same timeout plus unhandled rejection in 1s. Source inspection confirms the test awaits `siblingEntered` before attaching rejection handling to `run`, while the throwing frontend can terminalize before backend verifier entry; no `finally` releases the sibling on early failure/timeout. Five unmodified isolated runs passed, demonstrating schedule sensitivity. Add a deterministic two-arrival barrier/early rejection handling and unconditional bounded release/join in the test fixture only; preserve production F-03 semantics. Require causal RED/GREEN, both parameter rows, repeated focused/full contention, literal local check and hosted Node 22 success. `T,C,I`. | **100% diagnosis, 0% correction**; 0/3; not audited |
...
|---|---|---|---|---|
| `UI-03` | 0.4, 11.1–11.2, 11.7 | **INTEGRATED + BROWSER-AUDITED** at `83954f7`; final hosted `C` blocked by `TST-03` | Create Agent's transparent preset radios inherited global viewport-wide input sizing, expanding document width to 2299/1280 and 2579/1440. The integrated correction bounds only the hidden radio box and adds a theme-matched visible card focus outline. Auditor independently passed focused Chromium 2/2, full browser 4/4 and harness unit 2/2 at both required viewports; document/body X/Y, exact neutralized 1x1 boxes, accessible group/names, Generalist default, one Tab stop, arrow focus/selection, visible 2px outline, all label clicks and exactly one checked radio passed. Regenerated screenshot hashes matched the committed images; visual review preserved layout/theme. Integrated local check passed strict test-source and production typechecks, launcher 3/3, server 563/563 with one opt-in live skip, and both builds. Hosted run `33261198788` failed only the schedule-sensitive F-03 regression now tracked as `TST-03`; UI/browser tests were not part of that workflow. `T,C,B,U,I`. | **98% scoped**; `T,B,U,I` passed (4/5), hosted `C` pending; review required |
...
| `E2E-01` | 11.6.1, 17 baseline | **BLOCKED only by `TST-03` hosted baseline**; UI-03 browser gate cleared | After `TST-03` restores the required hosted gate, resume the authenticated create → fake Playground artifact/test → follow-up/session → server restart continuity journey at both required viewports. The Create Agent no-overflow and radio accessibility prerequisite is independently browser-audited; retain it as the first regression gate. No live call is permitted until deterministic acceptance is green. `C,B,L,S,U,I`. | **35%**; 0/6 journey gates; not audited |
