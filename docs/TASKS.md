...
**Latest protected-main integration:** `53f2529` (PR #97), closing issue #96's
exact-main release-readiness remediation. The tested product head was `223a5fe`; the
final PR head was `14b69f2`. A clean post-integration worktree at the exact merge
passed the repository, coverage, browser, Terraform and zero-spend live-preflight
gates, and protected-main workflow run `33396729636` passed on that SHA. Independent
UI/security review, external Ark inference and demo rehearsals remain separate open
evidence. Exact observations are in `BUILD_LOG.md` and
...
|---|---|---|
| Environment | Node `26.5.0`; npm `11.17.0`; Playwright `1.62.1`; Docker client/server `29.6.1` | Recorded on merge `53f2529` |
| Workspace typecheck | Both server and web passed during `npm run check` | Pass |
| Root script/deploy tests | 26/26 passed | Pass |
| Server suite | Merge `53f2529`: 856 passed, 3 explicit opt-in skips | Pass integrated |
| Web unit suite | 20/20 passed | Pass integrated |
...
| Coverage | Server 86.15/80.05/91.46/88.03%; browser 88.40/80.00/87.35/92.37% (statements/branches/functions/lines) | Pass integrated; all four metrics >=80% and every production file required |
| Dependency audit | `npm audit --json`: 0 vulnerabilities | Pass |
| Script syntax | `bash -n` launcher and Node launcher syntax passed | Pass |
| Full browser matrix | 48/48: 24 functional flows at both `1280x800` and `1440x900` | Pass integrated; independent `U` remains |
| Terraform | Pinned Terraform `1.9.8` plus Volcengine provider `0.0.58` initialized and validated a disposable copy | Pass integrated |
| Live Runtime preflight | Exact-tree Runtime/controller build, non-root controller, named volume/socket, one test discovered, cleanup | Pass integrated; zero model requests |
| Hosted protected-main gate | `Required checks` push run `33396729636` passed on merge `53f2529` | Pass integrated |
...
| `OPS-06` | 6.1, 8.10, 17 baseline | **COMPATIBILITY INTEGRATED** through `53f2529`; original implementation `b80af11` ([PR #56](https://github.com/kyashp/shepherd/pull/56)) | The bounded diagnostics and volume-aware Runtime path pass on the affected macOS host. The carried compose-exposure High was closed as `SEC-06`: the base profile defaults publishing to loopback while preserving explicit `PUBLIC_BIND_ADDR` opt-in, and public effective binds require a strong token. PR #97 additionally makes the exact-tree live gate use a named state volume and non-root controller, eliminating the host-share sandbox-probe blocker without weakening sandbox, cleanup or credential boundaries. Historical implementation and review detail remains in `BUILD_LOG.md`; external model execution is owned separately by `LIVE-01`. | **100% scoped**; `T,A,C,S,I` passed; integration verified, not independently audited |
...
**Issue #96 integration overlay:** PR #97 merged as `53f2529` and extends the
existing rows without rewriting their earlier audit history. `UI-GATE` passes an
...
independent `U` remains. `SEC-REVIEW` integration bounds durable retention, makes
Agent creation atomic, protects tokenless loopback/same-origin access, refuses a
public bind without a token, and removes runtime credentials from Terraform
input/cloud-init/state; the required independent part-5 review remains. `LIVE-01`
has a passing exact-tree named-volume Docker preflight on the affected host; the
external provider turn remains unexecuted pending informed approval for sending
repository-derived payloads to Ark. Deterministic protected-main `I` passed on the
merge; neither `U` nor external `L` is inferred from it.
...
| `UI-GATE` | 0.4, 11.1–11.7 | **INTEGRATED** through `53f2529` via [PR #97](https://github.com/kyashp/shepherd/pull/97), [issue #45](https://github.com/kyashp/shepherd/issues/45); earlier causal UI fixes `bbc7c81`; independent `U` remains | The real-server Playwright matrix now covers 24 functional flows at both exact viewports: the six PRD surfaces plus auth/shell, full Agent CRUD/lifecycle/edit hydration, Settings persistence/reset/recovery, direct Playground, Project Group, successful/denied Shepherd Missions, loading/empty/reconnect/malformed/not-found states, durable restart, verifier/runtime contracts, contrast and truthful promotion evidence. Exact merge `53f2529` passed 48/48 with no document-level overflow; Web 20/20, coverage, strict types/builds and protected-main run `33396729636` pass. The accepted design was preserved. `C,B,U,I`. | **95% scoped**; `C,B,I` passed (3/4), independent `U` pending; not audited |
...
| `SEC-REVIEW` | 0.3, 13, 17 | **PARTIAL, REMEDIATION INTEGRATED** through `53f2529`; independent acceptance tracked in [issue #89](https://github.com/kyashp/shepherd/issues/89), successor to [#49](https://github.com/kyashp/shepherd/issues/49) | Parts 1-3 are evidenced at `bd9b093`. The seven-lens part-4 review confirmed 19 findings; its five Highs (`SEC-02`–`SEC-06`) are fixed with causal coverage. PR #97 closes the remaining concrete release-audit findings by bounding durable collections, making Agent publication atomic, enforcing tokenless loopback Host plus same-origin Origin, requiring a strong token for every effective public bind, and keeping runtime credentials out of Terraform input/cloud-init/state. Exact merge `53f2529` passes the deterministic security regressions, dependency audit, coverage, Terraform validation and protected-main workflow. Issue #89 remains open because the required independent part-5 security review cannot be self-certified; four documented Low findings also remain review-visible. `C,S,I`. | **90%**; concrete High/Medium remediation and `C,S,I` integrated, independent acceptance pending; not audited |
...
| `LIVE-01` | 16 Phase 3/7/8 | `PARTIAL`; external provider turn awaits informed approval; [issue #65](https://github.com/kyashp/shepherd/issues/65), split out of #47 | Reviewer sub-gate is satisfied: command attempt two made exactly one external request and completed with two bounded findings while deterministic outcome/security/cleanup stayed green. PR #97 removes the affected-host bind-mount blocker: exact merge `53f2529` builds the current Runtime and non-root controller, uses a named state volume plus engine socket, discovers exactly one bounded live integration test, and cleans temporary resources. The zero-spend preflight passes on the affected macOS host. Current live Shepherd Mission and legacy continuity still require the explicitly approved external Ark turn; preflight is not substituted for model evidence. `L,S,I`. | **70%**; `S,I` and construction preflight pass, external `L` pending; not audited |
...
| `DEL-02` | 16 Phase 9C | **INTEGRATED** at `53f2529` via [PR #97](https://github.com/kyashp/shepherd/pull/97) | [`SHEPHERD_TEST_REPORT.md`](SHEPHERD_TEST_REPORT.md) records the exact base/product/merge heads, commands, failures, post-integration counts, coverage, browser/container/Terraform scope, and explicit live/review/rehearsal limitations. | **98%**; `C,I` passed; independent final package review not claimed |
| `DEL-03` | 16 Phase 9D | **INTEGRATED** at `53f2529` via [PR #97](https://github.com/kyashp/shepherd/pull/97) | README documents reproducible coverage/Terraform validation, credential-safe Terraform deployment, and links the exact report without removing starter setup. | **85%**; frozen-state integration complete, independent final package review pending |
| `DEL-04` | 0.1, 16 Phase 9E | **INTEGRATION RECONCILIATION** at `53f2529` via [PR #97](https://github.com/kyashp/shepherd/pull/97); prior consolidation audited at `5a38d37` | `TASKS.md`, `SHEPHERD.md`, `LOCAL_POC.md`, `FIXES.md`, `BUILD_LOG.md`, README and the test report distinguish protected-main, candidate, live-external, independent-review and rehearsal evidence. | **98% overall**; protected-main reconciliation passes, independent document audit pending |
| `DEL-05` | 16 Phase 9E, 17 | **PARTIAL INTEGRATED** at `53f2529` via [PR #97](https://github.com/kyashp/shepherd/pull/97) | Dependency, tracked-secret, generated-artifact, exact-tree Docker, Terraform, coverage, complete browser and hosted integration checks pass. External live inference, independent security/UI review, demo rehearsals and broader optional PRD rows remain. | **75%**; `C,B,S,I` pass, `L,U` pending; not audited |
...
- **Presentation-host caveat:** protected `main` no longer has the historical
  bind-mounted `LIVE-01` construction blocker. The exact-tree zero-spend preflight
  passes on the affected host. The external Ark request remains a separate consent
  and evidence boundary.
- **Current completion verdict:** incomplete. Merge `53f2529` closes the
  deterministic browser, coverage, Terraform, release-harness, identified security
  and protected-main integration gaps, but independent `U`, external `L`, and three
  rehearsals remain explicit. The older 82% estimate is retained only
