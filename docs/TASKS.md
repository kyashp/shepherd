...
| `F-06` | 9.1, 12: persistence error | **FIXER CANDIDATE; Auditor pending** | Representative recovery design is unchanged; recovery read operations are bounded and primary/journal double-fault temps are exact-owned, strictly validated and removed before restart state is observable. | **90% scoped**; local T/C/S candidate, Auditor/hosted I pending |
| `TST-21` | 0.3, 9.1, 12, 13 | **FIXER CANDIDATE; Auditor pending** | Four read fault stages, two primary+unlink double faults and two primary+unlink+marker triple faults are causal. Primary identity is preserved with bounded `cleanupPending`; restart removes marker-owned temps and same-instance retry consumes exact in-memory ownership. | **90%**; focused 43/43 local; final full/security pending; not audited |
