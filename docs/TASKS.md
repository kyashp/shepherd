...
| `E2E-02` | 11.6.2, 15 | **FIXER-VERIFIED CANDIDATE**; Auditor integration pending | Authenticated real-server deterministic hero functionally proves Contracts/isolated Planes, independent verification, exact semantic collision, concurrent candidates from one immutable base, cookie pass/bearer fail, objective selection, distinct final re-verification, protected promotion, retained loser, Project Group lifecycle and reset/cleanup. TST-10 visibly proves promotion start; TST-11 visibly proves selected/rejected Candidate evidence while preserving final-promotion keyboard proof. Focused journeys passed 4/4 and all 13 named claims are visible at both viewports with 26/26 unique hashes. Full Chromium, harness, strict and two literal gates passed locally. Shepherd summaries remain truthful (`GC-06` open); no `UI-02` estimate or live/model behavior is claimed. `C,B,S,U,I`. | **95% scoped**; candidate functional/security/visual gates pass; independent hosted Auditor `I` remains |
...
| `TST-11` | 11.6.2, 15 | **FIXER VERIFIED**; Auditor pending | Test-only correction scrolls each exact selected/rejected Candidate summary inside the drawer and proves positive drawer/browser intersection before capture. Focused hero passed 4/4 journeys (52 captures); manual review confirmed 13/13 named claims at both viewports and 26/26 unique hashes. Stage 10 final evidence, TST-10, keyboard/focus and security assertions remain. Harness 7/7, full Chromium 12/12, strict test types and literal check twice passed; no product/UI/theme/timing change. | **90% fix confidence**; `T,C,B,U` passed (4/4); independent hosted `I` pending |
