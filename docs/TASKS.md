...
| `F-06` | 9.1, 12: persistence error | **FIXER STACK / blocked by TST-23** with TST-21/22 | Representative recovery design, raw-read closure and replacement private-directory marker state machine retain focused evidence. A test-owned Store queue must be joined before combined Auditor integration. | **90% scoped**; `T,C,S` candidate evidence; `A,I` pending |
...
| `TST-22` | 0.3, 9.1, 12, 13 | **FIXER CHECKPOINT**; blocked by TST-23 | Owner-validated `0700` private temp namespace plus pre-published fixed-schema marker makes publication/removal prefixes idempotent without touching general-parent lookalikes. Store 70/70 and security 0 High/Medium pass; adjacent/full await the TST-23 test-owned queue join. | **85% scoped**; `T,C,S` candidate evidence; `A,I` blocked |
| `TST-23` | 0.2, 9.1, 12, 16 Phase 9B | **OPEN / READY**; Fixer; test-only | The TST-17 attention row must join the original Store queue through a test-owned sentinel before independent reload and root teardown. Preserve immediate rejection observation, all lifecycle/security assertions and production fail-closed concurrency. | **25%**; exact isolated assertions 1/1 plus one unhandled RED; 0/3 correction gates |
