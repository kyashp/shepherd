...
| Server suite | 771 tests passed; 2 explicit opt-in tests skipped | Pass on general Contract-intake candidate PR #59; protected-main integration pending |
...
| `OPS-06` | 6.1, 8.10, 17 baseline | **DIAGNOSTICS INTEGRATED / AUDITED** at `68dbd59`; **compatibility candidate** on `fix/47-ops-06-container-state-volume`, [PR #56](https://github.com/kyashp/shepherd/pull/56) | The bounded stage/reason diagnostics did their job and the affected-host cause is now reproduced: `codex sandbox linux --full-auto` cannot enforce Landlock per-file access rights on a host share reaching the engine through a virtual machine, so the sandbox subprobe's first redirection is denied and the runner reports the catch-all `sandbox_probe_failed`. Both startup probes were also certifying nothing, because `touch` succeeds on exactly that filesystem. Correction adds the opt-in container state volume (`CONTAINER_STATE_ROOT` plus `CONTAINER_STATE_VOLUME`, required together, `volume-nocopy` mandatory), translates the independent verifier, and makes both probes write and read back bytes; unset keeps host bind mounts byte-identically. On the affected host the unmodified preflight now returns `SHEPHERD_RUNTIME_PREFLIGHT_OK` and a containerized non-root control plane completes the sibling-Runtime loop. Causal tests pass; `typecheck` and `build` pass. The full parallel suite is **not green on that host before or after the change** (pristine `main` 39 failed, branch 61 failed, all timeouts, `main`'s set a strict subset); serially the five heaviest files pass 167/168 with the one failure reproduced on `main`. A green full gate on a Linux host, the `container-volume` startup smoke, and Auditor `I` remain open. See `docs/BUILD_LOG.md`. | **90% overall**; `T,C` passed for the correction, cause reproduced on the affected host; `A,S,L,I` and a green full gate pending, so no compatibility claim yet |
