...
| `F-06` | 9.1, 12: persistence error | **FIXER STACK / blocked by TST-24** with TST-21–23 | Representative recovery design, raw-read closure and replacement private-directory marker state machine retain focused evidence. The recovery fixture must join intent cleanup and the original Store queue before combined integration. | **90% scoped**; `T,C,S` candidate evidence; `A,I` pending |
| `TST-21` | 0.3, 9.1, 12, 13 | **FIXER STACK / Auditor pending** with TST-22 | Four read faults remain fixed/cause-free; database and journal double faults now use the TST-22 idempotent marker authority for same-instance and restart cleanup. | **90% scoped**; candidate gates pass; not audited |
| `TST-22` | 0.3, 9.1, 12, 13 | **FIXER CHECKPOINT**; distinct full-gate blocker | Owner-validated `0700` private temp namespace plus pre-published fixed-schema marker makes publication/removal prefixes idempotent without touching general-parent lookalikes. Store 70/70, security 0 High/Medium and TST-23 combined 138/138 pass; a separate transient recovery-row quiescence defect blocks full integration. | **90% scoped**; `T,C,S` candidate evidence; `A,I` blocked |
| `TST-23` | 0.2, 9.1, 12, 16 Phase 9B | **FIXER CANDIDATE**; Auditor pending; test-only | One no-op Store sentinel after attention observation joins all prior original-store work before reload/teardown. Exact row 20/20 and combined slice 138/138 pass without late rejection/artifact; full gate stopped on a distinct recovery-row race. | **90% scoped**; `T,C` pass; `A,I` pending |
| `TST-24` | 0.2, 9.1, 12, 16 Phase 9B | **OPEN / READY**; Fixer; test-only; blocks F-06 | After recovery attention is visible, wait for the exact intent to clear and enqueue a no-op sentinel on the original Store before cleanup assertions or reload/cancel/teardown. Preserve all durable/no-promotion/security assertions. | **25%**; contention RED after 18 passes at iteration 19; 0/3 correction gates |
