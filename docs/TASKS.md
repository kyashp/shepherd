...
| `TST-13` | 0.3, 12, 13 | **FIX CANDIDATE** on `fix/f-01-02-runtime-redaction`; Auditor integration pending | Runtime errors now expose only fixed execution text or a validated numeric timeout. Closed/non-mutable typed kind/deadline survive runner→executor→Contract and candidate service paths; parsed output, stderr, create/spawn, opaque, secret/path and hostile-kind canaries are absent from Error inspection, durable/reloaded state, events and public DTO. Candidate failure proves no promotion/head movement. Focused 75/75, adjacent 226/226, strict tests, two literal checks and scoped security review pass; external calls 0. See FIXES/BUILD_LOG for exact evidence. | **98% scoped**; no High/Medium security finding; Auditor U/I pending |
