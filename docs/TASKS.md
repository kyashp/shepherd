...
| `TST-09` | 0.2, 16 live gate, 17 | **READY**; Fixer; blocks MR-03 `L` | The explicit reviewer-only live command inherits non-loopback `HOST` from `.env` and fails production HTTP-host auth validation before reviewer construction, although this isolated test starts no HTTP server. External calls remain 0. Force a safe loopback HOST for this explicit test process while retaining real Ark/SHEPHERD reviewer configuration and fail-closed predicate/filter; never weaken production auth, edit `.env`, mask reviewer auth/config failure, or auto-retry. | **25% scoped**; RED preserved; 0/4 `T,C,S,L`; not audited |
