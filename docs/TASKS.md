...
| `E2E-02` | 11.6.2, 15 | **BLOCKED** on `TST-10`; candidate `85550e0` remains unintegrated | Authenticated real-server deterministic hero functionally proves Contracts/isolated Planes, independent verification, exact semantic collision, concurrent candidates from one immutable base, cookie pass/bearer fail, objective selection, distinct final re-verification, protected promotion, retained loser, Project Group lifecycle and reset/cleanup. Auditor passed two focused invocations (4/4 browser plus harness unit 14/14), full Chromium 12/12, literal check and security/cleanup review. However, captures 05 and 06 are byte-identical in both viewports, so only 24/26 images are visually distinct and the promotion-reverification stage is not visible. Current truthful summaries remain Shepherd-authored (`GC-06` open); no `UI-02` estimate or live/model behavior is claimed. `C,B,S,U,I`. | **90% scoped**; functional/security gates pass, visual evidence and integration blocked by `TST-10` |
| `TST-10` | 11.6.2, 15 | **READY**; Fixer; owns `tests/e2e/shepherd-hero.spec.mjs` only | Make stage 06 visibly evidence the already-asserted promotion-start event: scroll its card into the contract pane, prove it intersects that pane, then capture. Do not change production/UI/theme, weaken the stage assertion, add delay/retry, or claim `GC-06`/`UI-02`. Acceptance: captures 05/06 differ at both viewports; focused hero passes twice per viewport with 52/52 reviewed observations, full Chromium/harness, strict/literal check, security/UI fallback, clean ignored artifacts and hosted integration. | **0% fix confidence**; RED reproduced 2/2 viewports; not audited |
