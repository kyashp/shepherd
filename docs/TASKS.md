...
| `E2E-02` | 11.6.2, 15 | **FIXER-VERIFIED CANDIDATE**; Auditor integration pending | Authenticated real-server deterministic hero functionally proves Contracts/isolated Planes, independent verification, exact semantic collision, concurrent candidates from one immutable base, cookie pass/bearer fail, objective selection, distinct final re-verification, protected promotion, retained loser, Project Group lifecycle and reset/cleanup. The TST-10 candidate makes stage 06 visibly evidence the exact promotion-start event; focused runs passed twice per viewport and the final corpus is 26/26 distinct images. Full Chromium, harness, strict and two literal gates passed locally. Current truthful summaries remain Shepherd-authored (`GC-06` open); no `UI-02` estimate or live/model behavior is claimed. `C,B,S,U,I`. | **95% scoped**; candidate functional/security/visual gates pass; independent hosted Auditor `I` remains |
| `TST-10` | 11.6.2, 15 | **FIXER VERIFIED**; Auditor pending | Test-only correction targets the exact `promotion_started` event, scrolls its card into `.event-list`, proves positive pane/viewport intersection, and asserts 05/06 PNG bytes differ. Focused hero passed 4/4 across two runs per viewport (52 capture observations); all 26 final images were reviewed and have distinct hashes. Harness 7/7, full Chromium 12/12, strict test types and literal check twice passed; no production/UI/theme/timing change. | **90% fix confidence**; `T,C,B,U` passed (4/4); independent hosted `I` pending |
