...
| `UI-04` | 11.1–11.3, 12 | `CANDIDATE`; Fixer; awaiting Auditor integration | Promotion-completed events now select and label only `promotionEvidence` as **Final promotion re-verification**; candidate events retain **Candidate verification**, and promotion-started claims no final evidence. Resolution Plane detail exposes both safe public stages through an existing-theme accessible tab selector with candidate default, one/none fallback, plane reset, roving arrows/Home/End, Tab/Enter/Space/focus-visible, drawer initial focus/Escape/opener restoration and internal scroll. Web helper/component 4/4, full Web 17/17, harness 7/7, focused browser 2/2 and full Chromium 10/10 at 1280/1440, strict/build and two final literal checks passed; public/private-field scans and independent UI/security re-reviews passed. Frontend/CSS/tests only; no backend/schema/live change. `T,C,B,U,I`. | **95% scoped**; `T,C,B,U` passed (4/5); Auditor integration/hosted `I` pending; not audited. `E2E-02` remains incomplete. |
