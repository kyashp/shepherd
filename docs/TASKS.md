...
| `F-05` | 8.4, 12: Git conflict | **FIXER CANDIDATE; Auditor pending** | Ordinary conflict retains bounded `git_conflict/integration_merge` only after abort plus clean/head proof. Unproved cleanup retains the integration Plane and active project under `attention_required` with fixed `git_conflict/integration_cleanup`; verified Contracts and released Agents remain truthful and downstream work is absent. | **90% scoped**; local T/C/S pass, Auditor I pending |
| `TST-18` | 0.3, 8.4, 12 | **FIXER CANDIDATE; Auditor pending** | Mandatory abort now precedes conflict-path validation; exit 128 is not accepted. Enumeration, abort and post-inspection injected rows assert fixed cause-free errors and exact resulting cleanup state; service evidence is bounded and no candidate/promotion occurs. | **90% scoped**; focused real-Git/service and full local gates recorded; not audited |
