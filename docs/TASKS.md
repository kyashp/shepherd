...
| `F-05` | 8.4, 12: Git conflict | **INTEGRATED / scoped behavior audited** at `f8d7f71`; global I held by TST-20 | Real conflict, cleanup attention, pre-merge identity and reset contracts passed local, security and hosted implementation gates. Final docs-head hosted run exposed an unrelated service-test teardown race, so repository stability is not 100%. | **95% scoped**; `T,A,C,S` 4/5; final I pending TST-20 |
| `TST-18` | 0.3, 8.4, 12 | **INTEGRATED / locally audited** at `f8d7f71`; final I held by TST-20 | Mandatory abort and truthful bounded cleanup evidence pass. | **95% scoped**; final hosted I pending |
| `TST-19` | 0.3, 8.4, 12 | **INTEGRATED / locally audited** at `f8d7f71`; final I held by TST-20 | Five pre-mutation identity mismatch rows preserve all target/source state across repeated real-Git runs. | **95% scoped**; final hosted I pending |
| `TST-20` | 0.2, 12, 16 Phase 9B, 17 | **HOSTED RED** run `33282108035`; Fixer | Infrastructure terminalization test releases a blocked sibling, waits only one `setImmediate`, then teardown removes its case root while late sibling/store-queue persistence remains active. Hosted Node 22 observed `JsonStore.persist` rename ENOENT under that exact root after the test returned. Join the exact sibling lifecycle/store completion before teardown; preserve fail-fast rejection and all F-03/TST03/04 assertions. No sleeps/retries/timeouts/suppression/cleanup catch. | **0%**; hosted RED 1/1; targeted row later 10/10 did not reproduce under isolation; not audited |
