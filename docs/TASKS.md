...
| `TST-14` | 0.3, 12, 13 | **READY**; Fixer; owns executor execution-home cleanup/error precedence tests | Bound `rm(privateHome)` failures without losing cleanup attempts. Raw cleanup errors must never escape; an existing cancellation remains cancellation and an existing timeout/execution kind remains unchanged, while cleanup-only failure becomes fixed typed execution failure. Prove opaque/secret/path canaries absent from Error/store/reload/event/API/Agent and prove retry/reconciliation. Preserve TST-12 preflight cleanup, OPS-06, F-03 and no promotion. | **0% fix confidence**; Medium RED source-confirmed; not audited |
