...
| `OPS-06` | 6.1, 8.10, 17 baseline | `CANDIDATE`; Fixer; collaborator/macOS validation required | The runner now returns a closed bounded stage/reason result instead of collapsing create/start/output/cleanup into false. Explicit exits distinguish non-root, private-home denial, Codex version and sandbox listen/connect/generic probe failures; unknown/raw engine errors become `engine_error`. Executor diagnostics expose only enums, never paths, commands, secrets, endpoints, prompts or stderr. Every original hardening gate remains fail-closed. Runner/executor 33/33, adjacent 87 plus one live skip, strict types, security review and full checks passed; no live/model call. `T,A,C,S,L,I`. | **80% scoped**; `T,A,C,S` passed (4/6); affected-macOS live `L` and Auditor integration `I` pending; not audited and no compatibility bypass claimed |
