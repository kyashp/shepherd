...
| ID | PRD ref | Status / owner / dependencies | Evidence and required acceptance | Confidence; gates passed; audit |
|---|---|---|---|---|
| `F-01/02` | 12: Agent timeout/runtime | **BLOCKED by `TST-15`**; corrected chain `3afc452` remains unintegrated | Closed typed Runtime identity and TST-13 public-message boundary pass across Contract/candidate durable/API surfaces. TST-14 correctly preserves success/cancel/timeout/execution precedence for execution-home cleanup, reconciles retained homes and prevents promotion/head movement. Auditor passed focused 81/81, adjacent 66/66 and literal check (launcher 3/3, Server 615 plus two explicit skips, Web 17/17, strict/build). Exhaustive cleanup review found raw sentinel open/close/unlink and interrupted-home/workspace reconciliation paths. No live/model/UI call ran. | **82% scoped**; F mapping/TST-13/14 pass; startup cleanup security/integration blocked by TST-15 |
| `TST-13` | 0.3, 12, 13 | **AUDITOR VERIFIED locally** at `c7b5175`; integration held by `TST-14` | Fixed execution text or validated numeric timeout survives the closed runner→executor→Contract/candidate boundary. Parsed stdout/stderr, create/start/spawn/container-cleanup, hostile typed fields, opaque/secret/path canaries are absent from Error/durable/reload/event/API/Agent surfaces; candidate attention has no promotion/head movement. Focused 75/75, adjacent 66/66, full check and scoped security passed for this boundary. | **95% scoped**; T/C/S pass, hosted I held by TST-14 |
| `TST-14` | 0.3, 12, 13 | **AUDITOR VERIFIED locally** at `3afc452`; integration held by TST-15 | Exact execution-home cleanup always runs; cleanup-only becomes fixed typed execution failure while cancellation/timeout/execution remain primary. Four-row Error/reconcile/retry matrix, Contract and candidate durable/store/reload/event/Agent/Fastify HTTP/log canaries pass; ownership releases and candidate has no promotion/head movement. | **95% scoped**; T/C/S pass for execution-home boundary; hosted I held by TST-15 |
| `TST-15` | 0.3, 6.1, 12, 13 | **READY**; Fixer; owns executor sentinel/reconciliation cleanup boundaries and tests | Bound sentinel open causes, validation/adoption close precedence, failed adoption unlink, interrupted private-home rm and preflight-workspace rm. Raw faults never escape; bounded primary error remains authoritative; cleanup-only/reconciliation failure is fixed and retained owned artifacts remain retryable. Prove String/stack/inspect/cause/log/startup surfaces clean. Preserve TST-12/14, OPS-06 and typed runtime behavior. | **0% fix confidence**; Medium RED source-confirmed; not audited |
