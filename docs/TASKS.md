...
| ID | PRD ref | Status / owner / dependencies | Evidence and required acceptance | Confidence; gates passed; audit |
|---|---|---|---|---|
| `F-01/02` | 12: Agent timeout/runtime | **BLOCKED by `TST-16`**; chain `8c7cfc1` remains unintegrated | Typed timeout/execution, TST-13 public messages, TST-14 execution-home precedence and TST-15 five-edge sentinel/reconciliation cleanup pass. Auditor focused 93/93 and adjacent 66/66. Full-source review found remaining raw executor-owned root/setup/enumeration filesystem/config operations in reconcile, preflight and run; these can escape startup or persist as unknown failures. No live/model/UI call ran. | **84% scoped**; TST13-15 pass locally; complete filesystem boundary/security/integration blocked by TST-16 |
| `TST-13` | 0.3, 12, 13 | **AUDITOR VERIFIED locally** at `c7b5175`; included in TST-15 chain candidate | Fixed execution text or validated numeric timeout survives the closed runner→executor→Contract/candidate boundary. Parsed stdout/stderr, create/start/spawn/container-cleanup, hostile typed fields, opaque/secret/path canaries are absent from Error/durable/reload/event/API/Agent surfaces; candidate attention has no promotion/head movement. Focused 75/75, adjacent 66/66, full check and scoped security passed for this boundary. | **95% scoped**; T/C/S pass; chain U/I pending |
| `TST-14` | 0.3, 12, 13 | **AUDITOR VERIFIED locally** at `3afc452`; included in TST-15 chain candidate | Exact execution-home cleanup always runs; cleanup-only becomes fixed typed execution failure while cancellation/timeout/execution remain primary. Four-row Error/reconcile/retry matrix, Contract and candidate durable/store/reload/event/Agent/Fastify HTTP/log canaries pass; ownership releases and candidate has no promotion/head movement. | **95% scoped**; T/C/S pass for execution-home boundary; chain U/I pending |
| `TST-15` | 0.3, 6.1, 12, 13 | **AUDITOR VERIFIED locally** at `8c7cfc1`; integration held by TST-16 | Closed stage/reason policy bounds sentinel pre-open/open/body/close/unlink and exact private-home/preflight-workspace rm. Five-edge/double-fault precedence, same-process retry, retained artifacts and containment pass. Restart with invalid retained sentinel is fail-closed and may require manual cleanup (documented Low availability residual). | **95% scoped**; T/C/S for named edges pass; hosted I held by TST-16 |
| `TST-16` | 0.3, 6.1, 12, 13 | **READY**; Fixer; owns complete executor filesystem/config boundary tests | Apply fixed typed stages to every remaining executor-owned mkdir/mkdtemp/lstat/readdir/realpath/chmod/config-write failure in root preparation, reconcile enumeration, preflight setup and execution setup. Preserve policy errors and primary-over-cleanup precedence. Prove opaque/secret/path canaries absent from Error/cause/log/startup and Contract/candidate/store/reload/event/Agent/API surfaces. Preserve exact containment, retained-artifact retry, TST12-15, OPS06 and runtime classification. | **0% fix confidence**; Medium raw boundary source-confirmed; not audited |
