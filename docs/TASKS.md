...
| `F-06` | 9.1, 12: persistence error | **BLOCKED / unintegrated** by `TST-22`; Fixer | Representative recovery design and raw-read closure pass focused review, but the cleanup-marker protocol cannot safely converge across same-instance retry and all crash prefixes. Correct TST-22 without broadening scope, then rerun the full matrix. | **80% scoped**; `T,A,C` evidence retained; independent `S` failed; `I` pending |
| `TST-21` | 0.3, 9.1, 12, 13 | **FIXER CANDIDATE** at `f65681a`; blocked by TST-22 | Four read faults are fixed/cause-free. Fresh-store restart removes marker-owned double-fault temps, but same-instance and incomplete marker lifecycle remain unsafe. | **70% scoped**; focused read 4/4 and fresh-restart 2/2; not audited |
| `TST-22` | 0.3, 9.1, 12, 13 | **OPEN / READY**; Fixer; blocks F-06/TST-21 | Make exact temp+marker cleanup idempotent for same-instance and every restart prefix. Preserve primary precedence and containment while proving missing/partial markers, removal faults, symlinks and unrelated files cannot retain secrets, delete outside scope, or permanently block startup. | **25%**; causal same-instance RED 2/2 plus independent Medium security finding; 0/4 correction gates |
