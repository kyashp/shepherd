...
| `F-05` | 8.4, 12: Git conflict | **BLOCKED by `TST-19`** on `audit/f05-premerge-identity`; Fixer | TST-18 cleanup boundaries pass, but `mergeCommit` checks the expected integration branch/head only after a conflict. A clean worktree switched to another branch/head can be mutated before detection. Prove identity before any merge mutation. | **80% scoped**; conflict/cleanup T/A/C/S pass, precondition/I fail; not audited |
| `TST-18` | 0.3, 8.4, 12 | **AUDITOR VERIFIED locally** at `53c8415`; integration held by TST-19 | Mandatory abort precedes validation; enumeration/abort/inspection failures use fixed cleanup evidence. Ordinary conflict proves absent MERGE_HEAD, clean status, exact observed pre-head and persisted branch; attention path retains truthful state and cancel→reset exact cleanup passed ephemerally. | **95% scoped**; `T,A,C,S` 4/5; hosted I held |
| `TST-19` | 0.3, 8.4, 12 | **SOURCE-CONFIRMED Medium** at `53c8415`; Fixer | Before `git merge`, require clean status, symbolic branch exactly equal to the persisted integration Plane branch, and actual HEAD exactly equal to its persisted head. Causally switch to a sibling branch and separately move HEAD; require fixed bounded failure before mutation and prove every Plane branch/ref/worktree/protected HEAD/canary and downstream state unchanged. | **0%**; independent security finding; not audited |
