...
| `F-06` | 9.1, 12: persistence error | **FIXER STACK / Auditor pending** with TST-21/22 | Representative recovery design and raw-read closure are retained; the replacement private-directory marker state machine closes the blocking cleanup lifecycle finding. Combined Auditor integration remains required. | **90% scoped**; `T,C,S` candidate evidence; `A,I` pending |
...
| `TST-22` | 0.3, 9.1, 12, 13 | **FIXER CHECKPOINT**; adjacent test-quiescence blocker | Owner-validated `0700` private temp namespace plus pre-published fixed-schema marker makes publication/removal prefixes idempotent without touching general-parent lookalikes. Store 70/70 and security 0 High/Medium pass, but the TST-17 reload row races its original Store queue (isolated failure on repetition 2), so the full gate is not claimed. | **85% scoped**; `T,C,S` candidate evidence; adjacent/full `A,I` blocked |
