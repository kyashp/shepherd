...
`de3e631361763a91272f044e94bbf7a50f1a7d98`
...
|---|---|---|---|---|
| `MR-01` | 4.1.13, 8.6, 12 | `LOCAL_AUDIT_CANDIDATE`; Integrator/Fixer | PR #16 merged to `main` as `403fc09`; audit branch `5fba12f` carries the provider-free integration plus the MR-02 blocker record, while the MR-02 correction awaits Auditor integration | Provider-free audit passed the bounded reviewer/service/config matrix, adjacent F-03/recovery/API/settings slices, literal check, harness, browser and security review. Integrate and audit the MR-02 candidate before hosting; defer `L` until MR-03 and one final sparse call. |
...
externally held; `MR-01` is local integration work awaiting audit of the MR-02
product-truthfulness correction. Historical
...
| `MR-02` | 4.1.13, 8.6, 11.5, 12 | `CANDIDATE`; Fixer; awaiting Auditor integration | The server now exposes only `shepherdModelReviewConfigured`, derived from the same fail-closed startup predicate used to compose the reviewer. Settings preserves the stored preference but renders a native disabled control plus explicit **Unavailable** process-scoped copy when false; it issues no PATCH. Configured mode retains normal mouse/keyboard PATCH behavior. Causal server 12/12, Web 5/5, reviewer/API matrix 71/71, harness 6/6, Chromium 8/8 (MR-02 2/2 at both viewports), strict test typechecks, two literal checks, audit 0 vulnerabilities and independent security/UI reviews passed; no live call. `T,A,C,B,S,U,I`. | **95% scoped**; `T,A,C,B,S,U` passed (6/7); Auditor integration/hosted `I` pending; not audited |
...
- **Current fixer candidate:** `MR-02`; its exact no-Ark browser/API/source failure
