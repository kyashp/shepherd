...
| `F-06` | 9.1, 12: persistence error | **FIXER STACK / full gate blocked** with TST-21/22 | Representative recovery design, raw-read closure and replacement private-directory marker state machine retain focused evidence. TST-23 closed its Plane-unwind queue race, but the transient recovery row separately observed attention before reconciliation completion under full-suite contention. | **90% scoped**; `T,C,S` candidate evidence; `A,I` pending |
