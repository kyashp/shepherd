...
| `OPS-06` | 6.1, 8.10, 17 baseline | **FIXER-VERIFIED CANDIDATE**; collaborator/macOS and Auditor integration required | The runner returns closed bounded stage/reason results and keeps every hardening gate fail-closed. TST-12 removes the remaining executor cleanup `Error.cause` leak: cleanup now yields only fixed `stage=cleanup reason=cleanup_failed`, while the primary runner diagnostic remains unchanged. Causal secret/path/OS inspection tests, adjacent 95+1 skip, strict types and two literal gates passed; independent security fallback passed 34/34 with no finding. No live/model call ran. `T,A,C,S,L,I`. | **80% scoped**; candidate `T,A,C,S` pass; affected-macOS `L` and integration `I` pending |
| `TST-12` | 0.3, 6.1, 13 | **FIXER VERIFIED**; Auditor pending | Executor-only correction discards raw mount-cleanup cause and throws fixed bounded cleanup stage/reason. A planted failure proves message/String/stack/inspect/JSON/cause surfaces contain no secret/private-path/OS detail; fail-closed cleanup, retry and primary typed diagnostic remain. Targeted 3/3, adjacent 95+1 opt-in skip, strict test types and literal check twice passed; independent security fallback passed executor/runner 34/34, H/M/L 0/0/0. | **95% fix confidence**; `T,C,S` passed (3/3); independent hosted `I` pending |
