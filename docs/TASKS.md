...
| `F-06` | 9.1, 12: persistence error | **BLOCKED / unintegrated** by `TST-21`; Fixer | The representative post-Contract/pre-integration journal design and its focused matrix pass, but journal open/stat/read native failures remain raw and primary+temp-unlink double faults can retain untracked serialized-state/journal temps. Close those boundaries without broadening the explicitly representative recovery scope, then rerun the full F-06 matrix. | **80% scoped**; Worker `T,A,C` evidence retained; independent `S` failed; `I` pending |
| `TST-21` | 0.3, 9.1, 12, 13 | **OPEN / READY**; Fixer; blocks F-06 | Bound all non-absent journal read/metadata failures without cause/path leakage. Preserve primary-error precedence while truthfully tracking and retrying exact database/journal temp cleanup after double faults; prove no artifact accumulation or unrelated deletion across retry/restart. | **35%**; causal source/security RED confirmed; 0/4 correction gates; not audited |
