...
| `UI-01` | 11.2, 12 | **REVIEWED CANDIDATE** on `fix/43-ui-01-unauthorized-denial`; [issue #43](https://github.com/kyashp/shepherd/issues/43); `FM-01` prerequisite integrated at `2e405810` | Ordinary failed Missions now reuse the attention panel to expose the precise denied Contract code/stage, durable state, full-ID affordance and explicit no-promotion outcome without CSS or layout changes. Focused presentation 6/6, Web 20/20, literal check, both required viewports, screenshot review and independent UI/security re-review passed. Protected-main integration remains. `T,C,B,U,I`. | **98% scoped**; `T,C,B,U` passed (4/5), `I` pending; not audited |
...
| `E2E-04` | 11.6.4, 12 | **REVIEWED CANDIDATE** on `fix/43-ui-01-unauthorized-denial`; [issue #43](https://github.com/kyashp/shepherd/issues/43); `FM-01` prerequisite integrated at `2e405810` | A real compiled service run attempts an out-of-scope `policy.json` change, is rejected at Contract authority, reloads through the production server, and proves typed visible denial, unchanged stored/actual protected heads, zero Candidates/integration/promotion, keyboard/focus, bounded evidence and no overflow at both required viewports. Focused Chromium passed 2/2; full harness 22/24 retains two isolated unchanged starter-kit failures. Independent UI/security re-review passed. `C,B,S,U,I`. | **98% scoped**; `C,B,S,U` passed (4/5), `I` pending; not audited |
