...
|---|---|---|---|---|
| `GC-01` | 4.1.12, 11.5–11.6.3 | `CANDIDATE`; TST-07 corrected on Fixer branch; PR #9 is merged to `main` | The complete eight-case formatter/parser/component regression now belongs to Web and is executed/typechecked by the root gate. Focused Web 8/8, Server routing 10/10 and two literal checks passed. Auditor must still rerun the two-viewport browser interaction and integrated/hosted gates before GC-01 closes. |
...
| `TST-07` | 0.2, 16 Phase 9B, 17 | `CANDIDATE`; Fixer correction pending Auditor integration | Exact TS6142/TS6059 RED was preserved. The complete eight-case file moved from Server to Web; Web gained explicit Vitest and strict test-source configs/scripts, and root `test`/`typecheck` now execute both workspaces. Focused Web passed 8/8, adjacent Server routing 10/10, both strict test projects and Web production type/build passed. Literal `npm run check` passed twice with launcher 3/3, Server 563/563 plus one opt-in skip, Web 9/9 and both builds. Server JSX/rootDir, product code and assertions are unchanged. `T,C,I`. | **96% scoped candidate**; `T,C` passed (2/3); `I` pending; not audited |
