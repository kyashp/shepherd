...
| `FM-01` | 7.6–7.7, 8.9, 12 | **WORKER CANDIDATE** at `051b32a` on [PR #72](https://github.com/kyashp/shepherd/pull/72), [issue #42](https://github.com/kyashp/shepherd/issues/42) | Preserved RED showed four Contract causes collapsing to `unknown` and final failed evidence omitted. The six-case matrix now proves exact bounded Mission/Contract-or-Candidate/Plane/event evidence through service, API and durable reload, plus stored and actual Git protected-head immutability. Targeted 6/6, adjacent database/service/promotion/recovery 181/181, and Server production/test types pass. The literal hook reached Server 806 passed plus three skips but retains two unrelated parallel-only interrupted-policy cleanup failures in `general-project.test.ts`; that file passes 8/8 alone, so `C` is not claimed. UI remains E2E scope. | **92% scoped**; `T,A` passed (2/4), `C,I` pending; not audited |
