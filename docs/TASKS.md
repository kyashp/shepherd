...
| `E2E-02` | 11.6.2, 15 | **BLOCKED** by `TST-11`; corrected chain `79da51a` remains unintegrated | Authenticated real-server deterministic hero functionally proves Contracts/isolated Planes, independent verification, exact semantic collision, concurrent candidates from one immutable base, cookie pass/bearer fail, objective selection, distinct final re-verification, protected promotion, retained loser, Project Group lifecycle and reset/cleanup. TST-10 makes stage 06 visibly evidence promotion start, and all 26 hashes are distinct. Auditor passed focused 4/4, harness unit 14/14, full Chromium 12/12, literal check and security/cleanup review. Exhaustive semantic screenshot review found stages 09 and 11 show only Plane metadata: their named candidate-verification evidence is below the drawer viewport. Shepherd summaries remain truthful (`GC-06` open); no `UI-02` estimate or live/model behavior is claimed. `C,B,S,U,I`. | **90% scoped**; functional/security gates pass; visual evidence and integration blocked by `TST-11` |
| `TST-10` | 11.6.2, 15 | **AUDITOR VERIFIED locally** at `79da51a`; integration held by `TST-11` | Test-only correction targets exact `promotion_started`, scrolls its card into `.event-list`, proves positive pane/viewport intersection, and asserts 05/06 bytes differ. Auditor focused 4/4 and 52/52 inspection confirmed both stage-06 markers; 26/26 hashes are distinct. Full harness/check/security/UI passed. | **95% scoped**; `T,C,B,U` 4/4, hosted `I` held with E2E-02 |
| `TST-11` | 11.6.2, 15 | **READY**; Fixer; owns `tests/e2e/shepherd-hero.spec.mjs` only | Make stages 09 and 11 visibly evidence their exact candidate verification, not only Plane metadata. Scroll the selected and rejected evidence/tab region inside the drawer, prove marker intersection with drawer and browser, then capture. Preserve stage 10 final evidence and TST-10. No production/UI/theme, delays/retries, assertion weakening, `GC-06` or `UI-02` claim. Acceptance: exhaustive 13-stage semantic manifest passes twice per viewport (52/52), all claimed distinct stages have unique hashes, full Chromium/harness, strict/literal, security/UI/cleanup and hosted integration. | **0% fix confidence**; RED reproduced 4/4 stage/viewport pairs; not audited |
