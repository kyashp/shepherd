...
| `F-04` | 12: worktree creation | **WORKER VERIFIED** candidate on `work/mock-main`; Auditor review pending | Initial Contract Plane creation failure now persists bounded `worktree_creation_failure`/`plane_creation` on the owning Contract and Mission, releases the logical Agent/project, emits public event/API evidence, survives reload, and creates no Plane. Execution, verification, integration, collision, candidates and promotion never start. Causal destination-collision and real partial-Git tests prove managed directory/worktree/branch rollback, protected HEAD/canary preservation and no raw path exposure. RED 1/1 became GREEN; focused 2/2, adjacent 107/107, strict types and two literal checks passed (server 663 plus two opt-in skips, Web 17/17). No live/model/UI/schema change. `T,A,C,S,I`. | **95% scoped**; Worker passed 5/5; independent Auditor/security/hosted integration pending |
