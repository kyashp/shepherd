...
| `E2E-02` | 11.6.2, 15 | **WORKER VERIFIED** candidate on `work/mock-main`; Auditor review pending | Authenticated real-server deterministic hero now gates and proves Contracts/isolated Planes, independent verification, exact semantic collision, concurrent candidates from one immutable base, cookie pass/bearer fail, objective selection, distinct final re-verification, protected promotion, retained loser, Project Group lifecycle and reset/cleanup. Both required viewports passed twice (4/4 focused); full Chromium passed 12/12 and harness unit 7/7. Twenty-six ignored screenshots were inspected with no clipping/theme drift. Security assertions cover 401, bounded no-network verifier ledger (11/11/11 create/complete/remove), public DTO/DOM/store/log canaries, canonical repository/output, no outside writes, no tracked `.shepherd`, and exact port/process/container/root cleanup. Current truthful summaries are Shepherd-authored (`GC-06` remains open); no live/model call. `C,B,S,U,I`. | **95% scoped**; Worker passed 5/5; independent Auditor `I/U/S` and hosted candidate run pending |
