...
| `F-04` | 12: worktree creation | **FIXER VERIFIED candidate** on `fix/f04-plane-unwind`; Auditor pending | First-call bounded failure/partial-Git rollback and transactional second-call batch unwind pass. Only current-batch Planes are destroyed/removed; original failed Contract/Mission cause remains. Cleanup failure stays discoverable in bounded attention without claiming F-06 persistence semantics. | **95% scoped**; T/C/S local pass; U/I pending |
| `TST-17` | 0.3, 6.1, 12 | **FIXER + security review VERIFIED candidate** on `fix/f04-plane-unwind`; Auditor pending | Second-of-two failure leaves zero durable/physical batch Planes, planeIds, worktrees or branches; protected HEAD/canary and unrelated state remain. Cleanup double-fault retains exact Plane/Contract evidence under `attention_required`, releases Agents, exposes no raw path, and does not use `persistence_error`. | **95% scoped**; causal 2/2, service/Git 61/61, adjacent 186/186, strict/two full checks/security pass; U/I pending |
