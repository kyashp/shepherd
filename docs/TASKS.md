...
| `F-05` | 8.4, 12: Git conflict | **AUDITED** at `f8d7f71` | Real conflicts persist bounded clean inspection evidence; unproved cleanup retains bounded attention evidence. Pre-merge identity proves absent MERGE_HEAD, clean status, exact persisted branch/head before mutation. Reset removes exact retained resources and protected HEAD remains unchanged. | **100% scoped**; `T,A,C,S,I` 5/5; audited |
| `TST-18` | 0.3, 8.4, 12 | **AUDITED** at `f8d7f71` | Mandatory abort precedes validation; enumeration/abort/inspection failures are cause-free and cleanup state is truthful. Ordinary conflict proves clean/head/branch; attention cancel→reset exact cleanup passes. | **100% scoped**; `T,A,C,S,I` 5/5; audited |
| `TST-19` | 0.3, 8.4, 12 | **AUDITED** at `f8d7f71` | Detached, sibling branch, moved head, dirty target and preexisting merge fail before mutation. Target HEAD/status/all refs/MERGE metadata and source HEAD remain exact across repeated real-Git runs. | **100% scoped**; `T,A,C,S,I` 5/5; audited |
