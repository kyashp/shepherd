...
|---|---|---|---|---|
| `MR-03` | 8.6, 12 | `LOCAL_AUDIT_BLOCKED`; live configuration | PR #25 merged to `main`; local chain ends at `f1e574a` on `audit/tst-08-mr03-cleanup` | All deterministic/security gates passed. The sole live command failed before egress because non-loopback `APP_AUTH_TOKEN` was placeholder/too short; external calls 0 and retry prohibited. Supply a valid local auth token for a future newly authorized single attempt, then integrate/host. |
...
| `TST-08` | 0.2, 12, 16 Phase 9B, 17 | **CANDIDATE**; Fixer; awaiting Auditor integration | Literal check on local MR-03 merge `3c878b3` passed product assertions but failed teardown when an unjoined sibling removed its exact `.trusted-verification/verify-*` snapshot. The test-only candidate applies the TST-04 two-arrival barrier, immediate rejection observer, exact `VerificationRequest.planePath` containment checks, pre-release exact-basename removal watcher, and finally joins Mission/sibling/removal. Exact row passed 20/20, both infrastructure rows 40/40, MR-03 matrix 164/164, adjacent service/container/API/store/recovery 82/82, strict test types and two literal checks (Server 591/591 plus two live skips; Web 13/13). No production/cleanup/MR-03 change or live call. | **90% scoped**; `T,C,S` passed (3/4); Auditor integration/hosted `I` pending; not audited |
| `TST-09` | 0.2, 16 live gate, 17 | **CANDIDATE**; Fixer; awaiting Auditor review before MR-03 `L` | The reviewer-only live test now passes `loadConfig` a cloned environment with only `HOST` forced to loopback; it retains actual Ark key/model/base URL, SHEPHERD fallback/override, all other config, the production reviewer predicate, sensitive-value filter and fatal handling. Causal config coverage proves the original non-loopback weak-token source still rejects, the clone configures review, and the source is unchanged. Focused config/default-skip passed 27 + one skip; script remains double-gated, single-worker, retry 0; TST-08 passed 20/20 and 40/40; MR-03 matrix 165/165; strict tests and two literal checks passed (Server 592/592 plus two live skips; Web 13/13); independent security review and scans passed. External calls remain 0. | **90% scoped**; `T,C,S` passed (3/4); Auditor-authorized single live `L` pending; not audited |
...
| `MR-01` | 4.1.13, 8.6, 12 | Integrated/provider-free audited at `577255e`; blocked by MR-03 live configuration and final `L` | The bounded reviewer consumes only trusted verified summaries/evidence refs, validates schema and refs, records advisory/degraded evidence, and never changes deterministic collision/candidate/promotion authority. Provider-free and MR-03 deterministic gates passed. The one authorized live command made zero external calls because configuration failed before egress; no retry occurred. | **90% scoped**; `T,A,C,B,S,U,I` passed (7/8); `L` pending; not final audited |
...
| `LIVE-01` | 16 Phase 3/7/8 | `BLOCKED` by P0 and live configuration | Exactly one current live Mission plus legacy Playground continuity and exactly one live model-review smoke; bounded/redacted evidence only. The authorized reviewer command failed before egress on invalid non-loopback auth token, so its call count remains 0 and it is not satisfied. `L,S,I`. | **35%**; 0/3; not audited |
...
- **53 tracked work items:** 15 audited and 38 other incomplete; MR-03 is locally
  blocked by `TST-09` rather than externally held
