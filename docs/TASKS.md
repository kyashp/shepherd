...
| ID | PRD ref | Status / owner / dependencies | Evidence and required acceptance | Confidence; gates passed; audit |
|---|---|---|---|---|
| `F-01/02` | 12: Agent timeout/runtime | **FIXER VERIFIED chain; Auditor/integration pending** on `fix/f-01-02-filesystem-boundary` | Typed timeout/execution, fixed public messages, execution-home and sentinel/reconciliation precedence, and the complete executor filesystem/config boundary pass locally. TST-16 adds a 31-row fault matrix and 47-expression source inventory plus Contract/candidate durable/public proof. No live/model/UI call ran. | **95% scoped**; TST13-16 local T/C/S candidate evidence passes; independent U/I pending |
...
| `TST-16` | 0.3, 6.1, 12, 13 | **FIXER + security review VERIFIED candidate** on `fix/f-01-02-filesystem-boundary`; Auditor pending | Fixed stages now cover every remaining executor-owned root preparation, reconciliation enumeration, preflight setup and execution setup operation. The 31-row injected matrix, frozen 47-expression inventory, Contract/API/store/reload/log and candidate/no-promotion canary tests pass; retained targets retry exactly and TST12-15/OPS06 remain green. Initial Medium test-coverage findings were corrected; security re-review found no material issue. | **95% scoped**; focused 67/67, adjacent 154/154, strict/full local gates pass; U/I pending |
