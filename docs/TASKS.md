...
| `PERF-01` | 14 | **CANDIDATE** on [issue #46](https://github.com/kyashp/shepherd/issues/46), based on protected `main` `baa6b2f`; #41 integrated at `b2285ab` and #43 at `fbc937f` | The deterministic compiled service plus real Chromium UI recorded one clean sample at each required viewport: persisted collision → visible 908/861 ms (limit `<=1.5s`); Plane creation 225/228 ms; Contract verification through collision 1,424/1,358 ms; candidate verifier starts 35/39 ms apart while both were held active; collision → promotion start 1,368/1,275 ms; total Mission 3,690/3,550 ms. The test also asserts all 11 verifier create records use `network=none` and `readOnly=true`, with 11 completed/removed lifecycles. It is deterministic-fixture evidence, not a live-provider, package-download, or three-run rehearsal claim. `B,I`. | **70%**; `B` passed, `I` pending; not audited |
