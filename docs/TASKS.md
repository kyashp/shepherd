...
| ID | PRD ref | Status / owner / dependencies | Evidence and required acceptance | Confidence; gates passed; audit |
|---|---|---|---|---|
| `CRUD-01` | 4.1.2, 11.1, Track 1 Agent CRUD | **REVIEWED CANDIDATE** on [PR #67](https://github.com/kyashp/shepherd/pull/67), [issue #66](https://github.com/kyashp/shepherd/issues/66) | The reproduced delete conflict classified unbound `general-contract` clarification drafts as durable history. The candidate permits only exact inactive `agent-<id>` Projects whose references are human-authored, unbound clarification messages; Contracts, events, Missions, Planes, other messages, and malformed/orphaned state still return the specific 409. Exact Project and workspace cleanup is identity-validated and journaled across the database boundary. Injected pre-publication and post-rename durability-unknown failures converge on restart according to the on-disk Agent/Project state; a racing follow-up cannot create a policy journal after deletion owns the Store mutation. RED and three security counterexamples became green; lifecycle/project 27/27, adjacent API/Shepherd 77/77, literal check (launcher 3/3, Server 789 passed + two opt-in skips, Web 18/18, strict types/builds), and authenticated Chromium 2/2 at 1280x800 and 1440x900 pass. UI review passed with no High/Medium finding and no Web/CSS delta. Security re-review and protected-main integration rerun remain. `T,A,C,B,S,U,I`. | **96% scoped**; `T,A,C,B,U` passed (5/7), `S,I` pending; not audited |
