...
| ID | PRD ref | Status / owner / dependencies | Evidence and required acceptance | Confidence; gates passed; audit |
|---|---|---|---|---|
| `F-01/02` | 12: Agent timeout/runtime | **BLOCKED by `TST-14`**; corrected chain `c7b5175` remains unintegrated | Closed typed Runtime identity correctly distinguishes timeout/execution across Contract/Plane/Agent/Mission/event/API/reload, releases ownership, preserves untyped unknown/cancellation/F-03/OPS-06/candidate/verifier paths and creates no promotion. TST-13 fixed arbitrary runner diagnostics across Contract and candidate paths. Auditor passed focused 75/75, adjacent 66/66 and literal check (launcher 3/3, Server 609 plus two explicit skips, Web 17/17, strict/build). Exhaustive security review found execution-private-home `rm()` can still leak raw path/OS detail and override cancellation or typed identity. No live/model/UI call ran. | **80% scoped**; mapping/TST-13 pass; cleanup precedence/security and integration blocked by `TST-14` |
...
| `TST-14` | 0.3, 12, 13 | **FIX CANDIDATE** on `fix/f-01-02-private-home-cleanup`; Auditor integration pending | Exact private-home cleanup always runs; cleanup-only becomes fixed typed execution failure while cancellation/timeout/execution remain primary. Four-row Error/cleanup/reconcile/retry matrix, Contract and candidate durable/store/reload/event/Agent/public/real-HTTP/log canaries pass; inactive cancel proves ownership release; candidate has no promotion/head movement. Executor/OPS06 21/21, focused/adjacent 108/108, strict tests, two post-correction literal checks and final scoped security review pass; external calls 0. | **98% scoped**; no High/Medium/material security gap; Auditor U/I pending |
