...
| Live-model network egress | Clarification plus environment limitation | **OPEN FOR FINAL LIVE GATE.** The sole authorized reviewer command failed before egress because non-loopback `APP_AUTH_TOKEN` was placeholder/too short; external calls remain 0 and no retry occurred. Live Agent/model-review calls necessarily reach the configured Responses API once validly configured. Default tests stay model/network-free; only bounded explicit live gates may use credentials. | `LIVE-01`, `MR-01/03`, `SEC-REVIEW`, `DEL-02` |
