...
| Supported isolation profile | Clarification plus environment limitation | **PARTIAL.** Shepherd's Plane/verifier/no-network guarantees apply only to the local Docker/Colima/Podman path; host-process and ECS starter paths must report unsupported isolation rather than imply parity. Linux Docker gates and bounded stage/reason diagnostics are audited. The affected-host cause is now reproduced and named: a host share reaching the engine through a virtual machine cannot carry Codex's Landlock per-file access rights, so `host-bind` state cannot support the sandbox there and the gate correctly refuses to start. The launcher detects this through its own probes and switches to an engine state volume automatically, which restores the guarantees. | `OPS-06`, `LIVE-01`, `SEC-REVIEW` |
| Control-plane engine socket in container-volume mode | Clarification plus accepted trust decision | **SCOPED.** In `container-volume` mode the control plane runs inside the engine and is given the engine socket so it can start sibling Runtime containers. Socket access is equivalent to control of the engine, so it materially widens the control plane's authority; it does not weaken any Agent boundary, since Runtime and verifier containers keep non-root, read-only, dropped capabilities, no-network verification and the private-home denial. The control plane stays non-root and joins only the socket's group. `host-bind` mode, the default, grants no socket. A scoped socket proxy is the documented hardening if this profile is ever exposed beyond a local operator. | `OPS-06`, `SEC-REVIEW` |
