...
| `ST-02` | Stored notification values remain visible, but native disabled controls label the unimplemented capability **Reserved** and **Unavailable**. On the `349897d` current-main base, the focused regression, literal full check (554 passed, 2 skipped), zero-vulnerability audit, and terminal Chromium proof at both required viewports passed without mutation, focus, PATCH, overflow, clipping, or runtime errors. | [Draft PR #13](https://github.com/kyashp/shepherd/pull/13), integrated merge commit `066009b6eda658eee9a66135224d69b3a4dae515` | Independent review, final push/required checks, merge, and post-merge verification. |
