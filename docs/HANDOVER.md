...
Do not treat `main` as complete merely because scoped deterministic gates are
...
| Project Group `GC-01` mention formatting and native activation on merged PR #9 | On merged `main` at `fae0852`, the default server-workspace regression passes 8/8 formatter/parser-round-trip/preservation/submission-lock/boundary cases; both workspace typechecks and production builds pass; terminal Playwright mouse, Enter, Space, focus, caret, deferred-POST lock, exact limit, and visual checks pass at exact `1280x800` and `1440x900` | **97%** within this mocked-API interaction scope; the unrelated full-suite process/shared-state instability and broader `GC-02..07`, accessibility, polling, and populated-history work remain |
...
composition, and every missing/partial row in the failure matrix.
...
| `GC-01` | Group Chat mention buttons | **Resolved on merged [PR #9](https://github.com/kyashp/shepherd/pull/9) at `fae0852`.** For canonically valid, identifier-unambiguous Agent directories, the merged code emits parser-safe normalized/trimmed JSON-quoted name mentions, falls back to the Agent ID when message normalization would alter the target, preserves multiline composer content, locks mention mutation during submission, and rejects over-limit insertion without losing the draft. | The prior `unknown_agent` paths from UI-generated whitespace names, including normalization-changing and post-NFKC-trim edge cases, are fixed; the reviewer-found submission race and length-boundary rejection are covered. | On merged `main`, the 8/8 default-workspace regression, both workspace typechecks and production builds, plus mouse, Enter, Space, focus/caret, deferred-POST lock, exact 2,000-character boundary, and exact `1280x800`/`1440x900` checks pass. Full `npm run check` was not rerun post-merge; its pre-merge attempt exposed unchanged process/shared-state failures. Canonical-length and cross-ID/name namespace validation remain `GC-07`; keep `GC-02..07` separate. |
...
| Project Group message display/polling | Read API, sorting, connection/error/empty states exist; draft PR #17 anchors the composer below a flexible scrollable history, and merged [PR #9](https://github.com/kyashp/shepherd/pull/9) adds parser-safe name/ID mention insertion with multiline draft preservation | Browser polling/reconnect and populated-history tests plus fixes for `GC-02..07`; `GC-05` still disables typing before a project exists; accessibility and independent UI review remain |
...
3. Complete Group Chat API/browser regressions for `GC-02..07`. For canonically valid, identifier-unambiguous directories, merged PR #9 covers `GC-01` quoted/escaped whitespace names, normalization-changing name fallback to Agent IDs, post-NFKC trim, formatter-to-production-parser round trips, multiline draft preservation, native mouse/keyboard activation, focus/caret, and both required viewports; its scoped post-merge gate passed at `fae0852`. Still prove a clean-start state, a message that produces a bounded Shepherd result, a targeted Contract, post-Mission behavior, manifest-derived Agent summaries, and canonical length/ID-name namespace validation at create/update boundaries.
...
| `UI-S05` | 11.5 Project Group, Agent, Create/Edit | **Defective/I-U; scoped Project Group layout and mention interaction verified** | Draft PR #17 proves clean-start composer placement/no-page-overflow; merged PR #9 is post-merge verified for mouse/Enter/Space mention insertion, focus, caret, and both viewports. `GC-02..07` and remaining Agent/form/Playground browser evidence remain. |
...
- Project Group messages are bounded and idempotent, and merged PR #9 fixes parser-safe mention-button insertion for canonically valid, identifier-unambiguous Agent directories. The complete chat workflow remains defective: unmentioned text is persisted without Shepherd processing/reply, the only executable `@Agent` path targets an existing active demo Contract, and create/update canonical-name validation is weaker than the parser directory contract. See `GC-02..07` in the canonical ledger.
...
- Project Group display/polling, bounded parser-backed routing attempts, parser-safe
  mention-button insertion on merged PR #9, and Contract links; end-to-end routing
...
remained separate from the scoped correction.

PR #9 subsequently merged as `fae0852`. On that exact merged `main`, the focused
server-workspace regression passed 8/8, both workspace typechecks passed, and both
production builds passed. Terminal Playwright fallback was used because the in-app
browser reported no available backend. At exact `1280x800` and `1440x900`, mouse,
Enter, and Space produced the expected 59-character multiline draft; focus, the
collapsed `59..59` caret, and the solid 2 px focus-visible outline were retained.
The deferred-POST lock, exact 2,000-character boundary, over-limit rejection,
single expected POST, no-overflow/layout bounds, and absence of console, page, or
request failures all passed. The temporary, visually inspected screenshots were
not committed and had these SHA-256 hashes:

```text
1280x800  980854CC1C489C91BE5EBAFD42B940044AEF167FABFE8FF13E2E65D546D7CEC6
1440x900  507CC82FE4F8C43E23A72BD022FF7987EAA2EF4518C20AD9007D3C05159363E6
```

The full `npm run check` was not rerun post-merge; the exact pre-merge non-green
result above remains the latest full-suite observation and is not represented as a
post-merge pass.
...
committed E2E harness/screenshot corpus, or independent UI review.
...
6. **Repair Project Group end to end**
   - Retain the parser-safe mention formatting and pure draft-preservation coverage for `GC-01` merged in [PR #9](https://github.com/kyashp/shepherd/pull/9).
   - Treat the scoped `GC-01` button/keyboard/focus/caret/viewport gate as complete: the post-merge rerun passed on `main` at `fae0852`.
...
Latest merged-main GC-01 evidence is recorded against `fae0852`: the default
server-workspace regression passes 8/8, both workspace typechecks and both
production builds pass, and the exact-viewport browser matrix passes. The full
`npm run check` was not rerun post-merge. Its latest pre-merge attempt exposed two
unchanged server failures under suite load (557 passed, 2 failed, 5 skipped); the
service case passes 1/1 alone and the recovery file rotates across checkpoints even
in isolation. Preserve those exact results rather than copying an older green
count. The 0-vulnerability audit result is also pre-merge evidence.
