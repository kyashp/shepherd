...
- **Owner/status:** Fixer / `RESOLVED, INTEGRATED at b80af11` (PR #56); `A` and `S`
  ...
  run. Adjacent `A` coverage is supplied by PR #68 and the independent `S` review
  by PR #69; that review returned FAIL against `e6a5878`, which removed the
  non-loopback `APP_AUTH_TOKEN` requirement in the same merge with no recorded
  review. Its High finding is only partly closed — the guarded deploy script is one
  of three documented compose entry points — and the remainder is carried into
  `SEC-REVIEW`.
...
| `TST-25` | Reproduced on macOS at `b80af11`: `npm run test:e2e:harness:unit` fails 1 of 7 with `fake-codex: workspace identity must be canonical`. `tests/e2e/harness.test.mjs:38` builds its workspace via `mkdtemp(path.join(os.tmpdir(), "shepherd-fake-codex-"))`; on macOS `os.tmpdir()` returns a symlinked `/var/folders/...` path, so the deliberate `realpath` identity assertion at `tests/e2e/fixtures/fake-codex.mjs:69` correctly rejects it. Host-dependent, not intermittent: it passes where `os.tmpdir()` is already canonical, which is why no hosted run catches it — this target is not part of `npm run check`. No safety or data impact; it is a false failure on the presentation host. | Canonicalize the test's own temporary root before `mkdtemp`, matching the existing idiom at `tests/e2e/support/test-app.mjs:93`. Do **not** weaken or remove the fixture assertion: it is the `TST-06` canonical-ancestor hardening. Causal test: assert the harness still rejects a non-canonical workspace while the canonicalized fixture run passes. | **RESOLVED** at `e06357c` (PR #71, issue #70); discovered during `OPS-06` post-merge reconciliation at `b80af11`. The fix canonicalizes the test's own temporary root with `realpath` before `mkdtemp`; the fixture assertion is unchanged. |
| `SEC-01` | Reproduced against the real `createApp` at `bd9b093` with `APP_AUTH_TOKEN` configured and no `Authorization` header: `GET /api/system` returns 401 while `GET /%61pi/system` returns **200** and executes the handler. The `onRequest` hook decided whether to authenticate from `request.url`, the raw request target, while Fastify percent-decodes static path segments before route matching, so any encoded spelling of the `/api/` prefix skipped the hook and the router still resolved the request. Every route was affected including destructive ones (`POST /%61pi/shepherd/demo/reset` removes Plane worktrees and restores protected HEAD; `DELETE /%61pi/agents/<id>` executed). Remotely reachable wherever the token is the only control, because `docker-compose.yml` binds `0.0.0.0` and publishes the port. Not introduced by any recent change; the existing auth tests only ever exercised canonical spellings, so 107 tests passed with the bypass live. | Decide the hook from the decoded target instead of the raw one, treating an undecodable target as protected rather than exempt, and keep the `/api/health` and `/api/auth` exemptions exact-match so a query string still fails closed. Do not widen the exemptions. | **RESOLVED** on `audit/49-sec-review-evidence`; causal test `protects API routes spelled with a percent-encoded prefix`; found by the `SEC-REVIEW` boundary audit |
| `TST-26` | Reproduced at `e552959`: `recovery.process.test.ts` fails 5/5 **in isolation on an idle machine**, 168.75s for 5 tests, each reporting `Timed out waiting for recovered` with empty stderr. Not host speed: raising the message timeout to 400s still timed out, and a direct probe showed the recover child exits **code 0**, sends **zero** IPC messages, and writes nothing to stderr or stdout. Cause is at `test-fixtures/recovery-process.ts`: `send({type:"recovered"...})` is followed immediately by `process.exit(0)`, but `process.send` is asynchronous, so the child can terminate before the IPC frame is flushed. The parent never receives the message, and because the exit code is 0 the harness's exit handler does not reject either, so it waits out the full timeout for a result that was computed correctly and never left the process. Passes on faster hosts and in Linux CI, where the flush wins the race — which is why it was previously recorded as a host-speed timing condition. | Exit from the `process.send` flush callback instead of the next statement. Separately, make the harness reject on a clean exit that never sent the awaited message, so the cause is reported immediately rather than as an anonymous timeout. Do not raise timeouts or add sleeps. | **RESOLVED** on `fix/48-recovery-ipc-flush`; 5/5 pass and the file drops from 168.75s to 52s; reverting the fixture fails 5/5 in 48s with `Fixture exited 0/null without sending recovered` |
| `SEC-02` | Reproduced over a real socket at `0ed41dd`: with `APP_AUTH_TOKEN` configured and no `Authorization` header, `GET /api/system` returns 401 while `GET http://127.0.0.1:PORT/api/system` returns **200 and reaches the handler**. HTTP/1.1 permits an absolute-form request target, and `request.url` is then the whole URI, which does not start with `/api/`, so the hook returned early while the router still matched the path component. Every `/api/` route is affected including the destructive demo reset. This is a sibling of `SEC-01` that the `SEC-01` correction did not close: that fix decoded the raw target but still tested the raw string. Four independent review lenses converged on it. **`app.inject` cannot express this request** — it normalizes the target — so the first regression test written for it reported a false pass. | Derive the decision from the parsed, decoded pathname (`new URL(request.url, base).pathname`), which handles origin-form, absolute-form and protocol-relative targets uniformly; keep the exemptions exact-match on the whole target so a query string or non-origin-form spelling of a public route still fails closed; treat an unparseable or undecodable target as protected. Test over a real socket, not `app.inject`. | **RESOLVED** on `fix/89-absolute-form-auth-bypass`; causal socket test `protects API routes requested in absolute form`; reverting to the raw-target test fails 2 cases, reverting to decoded-but-unparsed fails 1 |
| `SEC-03` | Reproduced at `0ed41dd`: `scripts/check-deploy-auth-token.mjs` gated its CLI entry point on `import.meta.url === \`file://${process.argv[1]}\``. That is not a URL comparison — a space, `#` or any non-ASCII character percent-encodes in `import.meta.url` — and on macOS the filesystem reports names in NFD while an argument is typically NFC, so even a correctly built file URL compares unequal. From a directory named `My Deploy Repo #1 é` the guard body never executed: no output, exit 0, and `deploy-existing-ecs.sh` proceeded to publish the Agent execution API on `0.0.0.0` with an empty token. The existing tests imported the module and never spawned it, so this shipped green through `npm run check`. | Resolve both sides through the filesystem (`realpath` of `import.meta.filename` and of `process.argv[1]`), which removes URL encoding and Unicode normalization from the comparison. Add a test that spawns the CLI from a directory whose name contains a space, a `#` and a non-ASCII character. | **RESOLVED** on `fix/89-absolute-form-auth-bypass`; causal test `the checker CLI refuses an empty token from a directory whose name needs escaping` |
| `SEC-04` | Source-evidenced at `0ed41dd`: `buildCodexArgs` appended the caller-supplied prompt, and on the resume path a stored thread id, as unseparated positional arguments — on the same argv that carries `--sandbox`. A prompt beginning with `-` is therefore parsed by the Codex CLI as an option. Demonstrated argv included `-cmcp_servers.x.command=...` and `-cmodel_providers...base_url=...` reaching the CLI as options. The repository already applies the `--` discipline to every Git invocation and feeds the ephemeral executor's prompt over stdin; the legacy Agent runner was the one path missing it. | Push `--` before the positionals on both the new-session and resume branches. | **RESOLVED** on `fix/89-absolute-form-auth-bypass`; `codex-runner` and `container-codex-runner` assertions updated to pin the separator |
| `SEC-05` | Reproduced at `0ed41dd` with the real `PlaneManager` in the integration fixture: mandatory promotion re-verification ran against the **live Plane worktree**, not the commit being promoted. `withVerificationSnapshot` had a fast path that byte-copied any `refs/heads/shepherd/*` worktree whose HEAD matched and which `git status` called clean — but that cleanliness test, `changedFilesSince`, and every authority check are **ignore-blind** (`--untracked-files=all` without `--ignored`; `ls-files --others --exclude-standard`). An Agent commits a `.gitignore` and writes files under it: they are invisible to the cleanliness test, to `validateChangedPaths` at import, at `commitPlane`, at the candidate authority check and at the gate's final authority recheck — yet they are copied into the verification snapshot, persist in the worktree, and are re-exported into the next execution workspace for that Plane. The tree the mandatory checks execute against was therefore a strict, agent-controlled superset of the tree being promoted. Ignore rules are agent-controlled, so this is a containment failure of the write-authority envelope itself. | Two independent corrections, each of which closes it alone: always materialize the exact commit for the verification snapshot, so the verified tree equals the promoted tree by construction; and make the authority queries ignore-aware (`--ignored=matching`, and drop `--exclude-standard`) so such paths are authority-checked like any other change and a Plane holding them is not reported clean. | **RESOLVED** on `fix/89-absolute-form-auth-bypass`; causal test asserts an ignored file under a committed `.gitignore` is absent from the snapshot; reverting **both** fixes reproduces, reverting either alone does not |
| `SEC-06` | The `OPS-06` `S` review's carried High, confirmed still present at `0ed41dd` and now closed. `docker-compose.yml` published `"${PUBLIC_PORT:-3000}:3000"` with no address prefix, binding every host interface, while `.env.example` ships `APP_AUTH_TOKEN=` empty and the bearer hook short-circuits entirely on an empty token. `README`'s quickstart is `docker compose up --build` against exactly that file, and told the reader that nothing else was needed to start — so the advice to set a token "for any deployment reachable beyond your own machine" never fired, because the reader had no reason to think the quickstart made it reachable. The guard that encodes this knowledge is invoked only from `deploy-existing-ecs.sh`, which the quickstart never runs. Realistic worst case on that profile: unauthenticated prompt-driven command and file execution in the container, read/write to the three bind-mounted host directories, use of whatever credentials `.env` supplies, and the destructive demo reset. On Linux the reachability is wider than operators expect, because Compose port publishing installs DNAT rules evaluated ahead of the filter chains where host firewall rules live. | Default the published address to loopback with an explicit opt-in — `"${PUBLIC_BIND_ADDR:-127.0.0.1}:${PUBLIC_PORT:-3000}:3000"` — mirroring `docker-compose.state-volume.yml`, which already does exactly this. An earlier decision rejected pinning to loopback because it would remove the ECS profile's intended reachability; a defaulted variable keeps that reachability as an explicit opt-in and makes only the default safe. Document it in `.env.example` and in both README paths. | **RESOLVED** on `fix/89-absolute-form-auth-bypass`; causal tests assert the resolved `host_ip` is `127.0.0.1` by default and `0.0.0.0` when opted in; restoring the unprefixed port fails both |
