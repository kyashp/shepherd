...
- **Status:** **REVIEWED CANDIDATE** on PR #67 / issue #66. Causal lifecycle/project
  tests pass 27/27, adjacent API/Shepherd 77/77, literal `npm run check` passes
  launcher 3/3, Server 789 plus two opt-in skips, Web 18/18 and both builds, and the
  browser journey passes 2/2 with no overflow. UI review reports no High/Medium;
  the first security review's three Medium persistence/race counterexamples now
  have passing causal restart tests; security re-review and protected-main
  integration remain.
...
|---|---|---|---|
| `CRUD-01` | Reproduced: clarification-only human drafts created no Contract/Mission/Plane but caused the delete API to return the durable-history 409. Initial review also reproduced pre-/post-publication cleanup splits and a follow-up policy-journal race. | Remove only exact unbound general clarification state; journal Project/workspace cleanup across database publication, reconcile from on-disk truth, serialize project operations, and retain durable-history denial/accepted UI. | **REVIEWED CANDIDATE** on PR #67; all three Medium counterexamples now pass, `S,I` pending |
